...
- OpenAI and Claude retain their reviewed E3 baseline; exact Kimi `cn` +
  `kimi-k2.6` and MiniMax `cn` + `MiniMax-M2.7` cells are also retained. Every
  other provider profile, route, or sibling model advertised by that release
  must separately pass its exact model/endpoint harmless-fake-MCP E3 matrix;
  offline compatibility is not a waiver.
