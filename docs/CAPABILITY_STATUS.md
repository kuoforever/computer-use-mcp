...
> **Status: current review dashboard, verified 2026-08-12.** This page is the
...
and one opaque issue/consume transition per process-local gate instance. It has
no serialized loader, command, persistence, provider request, or execution
port. The independent Agent Console, provider intent call, launcher,
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Formal Demo v1 | `YES` — the selected role-based product story and safety boundary are specified in [Formal Demo v1](FORMAL_DEMO_V1.md) | `PARTIAL` — internal inert v1 intent/scenario/profile/Scope contracts, exact registry pins, fail-closed compilation, and a typed local disclosure/exact-`COMPILE` permit gate exist; no Console, provider compiler/call, launcher, executable adapter, durable composition, or Receipt exists | `PARTIAL` — `GDA-DEMO-007A` contract round-trip/binding tests plus `GDA-DEMO-007B` exact route/profile/task/resume binding, display, one-gate issue/consume, tamper, concurrency, Unicode/size, and no-port tests are deterministic offline evidence only; there is no process-wide or durable exactly-once claim | `NO` | `NO` | `NO` | Do not activate from this dashboard. Select exactly one later slice only through [Project status](../PROJECT_STATUS.md); a future provider-intent request must revalidate current provider terms and atomically consume before send, the email adapter remains unselected, and no fixed-workflow, application-coverage, or Universal-showcase evidence transfers |
