...
| Operator UI | `YES` | `PARTIAL` — console yes/deny approval, internal fake-host status decisions, the checkpoint-to-view-model reducer, passive non-activating progress window, atomic live polling, fixed independent-run grouping, and lock-free validated campaign grouping exist; presence indicator, Decision Cards, public status tool, and notification bridge do not | `PARTIAL` — approval, status-decision, progress-reducer, passive-window, live-polling, multi-run grouping, and campaign grouping are offline verified, including stable double-read snapshots, corrupt-campaign isolation, structural redaction, deterministic attention priority, and 150 consecutive non-blocked heartbeat publishes | `NO` | `PARTIAL` — the [live-polling smoke](PROGRESS_POLLER_EVIDENCE.md) retained foreground, completed 400/400 paired checkpoint/campaign publishes through a held execution lock, regrouped one run into History, and moved a paused campaign from Active to Attention; it is not a long soak | `NO` | Deliver presence, then fake-only Decision Card view models; keep mobile delivery host-owned |
