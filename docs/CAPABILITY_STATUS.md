...
The planned [Formal Demo v1](FORMAL_DEMO_V1.md) does not exist as an
executable product surface. Its independent Agent Console, `TaskIntent`,
`DemoScenarioSpec`, `ApplicationRoleProfile`, generic Scope Sheet compiler,
scenario digest, launcher, GitHub/PDF/Excel/Word/email-draft composition, and
Formal Demo Receipt are all unimplemented and have no offline, provider,
desktop, or application evidence. In the Operator UI row, "console" means the
existing terminal/CLI approval surface, not that planned Agent Console. The
only implemented Pre-run Review remains the narrow, Host-fixed
`public-web-word` Scope Sheet; it is not a generic review or launcher.

| Capability line | Designed | Implemented | Offline verified | Provider verified | Desktop verified | Application verified | Next gate |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Formal Demo v1 | `YES` — the selected role-based product story and safety boundary are specified in [Formal Demo v1](FORMAL_DEMO_V1.md) | `NO` — no independent Console, intent/scenario/profile contract, generic review compiler, launcher, selected application composition, or Formal Demo receipt exists | `NO` | `NO` | `NO` | `NO` | Do not activate from this dashboard. If [Project status](../PROJECT_STATUS.md) explicitly authorizes the proposed first slice, begin with versioned offline contracts and fail-closed compilation only; do not inherit fixed public-web-word, application-coverage, or Universal-showcase evidence |
