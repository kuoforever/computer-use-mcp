...
| Hierarchical control | `YES` — [the closed node set, fail-closed propagation, `H1`-`H8` order, and uncertainty boundary](HIERARCHICAL_TASK_AND_BEHAVIOR_TREES.md) are fixed | `PARTIAL` — H1-H7 add immutable nodes, exact digests/CAS, pure next-leaf compilation, typed fresh facts, one exact BOSS observation template, and one separately reviewed observation/action/verification-observation/final sequence. H7 reuses the existing Runtime Executor plus sole Runner boundary; H4 and public Planner/Executor remain observation-only | `YES` — prior H1-H6 gates plus exact H7 shape, pre-store rejection, durable leaf ordering, approval binding, post-action verification debt, content-excluding final input, denial, defer, unknown outcome, dispatched error, WAL retention, zero replay, and isolated-application state change pass deterministically | `N/A` | `PARTIAL` — [H7 isolated-application evidence](H7_BOUNDED_SIDE_EFFECT_EVIDENCE.md) uses the production stores/compiler/Runner/ledger with injected desktop and approval ports; no real MCP, Windows desktop, provider, or external application is claimed | `N/A` | Preserve the exact H7 shape and existing Runner authority; any public or real-application widening requires separate review |
...
1. **Hierarchical control and verified learning:** H1-H7 and L0-L4 now fix the node,
   ...
   separately gated bounded side-effect sequence, and persistent LOW-only canary
   route binding. H8 is exact next after L4 publication. None of these phases
