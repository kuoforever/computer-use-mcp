...
| Operator UI | `YES` | `PARTIAL` — console yes/deny approval, internal fake-host status decisions, the pure checkpoint-to-view-model reducer, and the passive non-activating progress-window shell over an injectable native surface exist; presence indicator, live checkpoint polling, Decision Cards, public status tool, and notification bridge do not | `PARTIAL` — approval, status-decision, progress-reducer, and passive-window (foreground-unchanged in injectable form; structural redaction) paths have offline evidence | `NO` | `PARTIAL` — the [passive-window non-activation smoke](PROGRESS_WINDOW_EVIDENCE.md) drew, refreshed, moved, and re-topped the real window over synthetic records with the foreground HWND unchanged; live polling and real checkpoint content were not on screen | `NO` | Add atomic live checkpoint polling over the passive window, then multi-run grouping; keep mobile delivery host-owned |
