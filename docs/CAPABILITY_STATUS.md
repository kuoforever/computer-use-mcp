...
| Operator UI | `YES` | `PARTIAL` — console approval, an opt-in two-choice focus-taking Decision Card through the existing ApprovalPort, passive progress, and opt-in durable ordinary-run presence exist; richer business alternatives, broader presence wiring, public status, and notifications do not | `YES` — Decision Card tests cover fixed rendering, authority-yield ordering, exact request plus six Host digests, close/timeout/error/malformed/expiry/drift denial, and the sole Runner dispatch boundary; progress/presence tests cover redaction, native behavior, and teardown | `NO` | `PARTIAL` — model [offline evidence](DECISION_CARD_MODEL_EVIDENCE.md), native [Decision Card focus/timeout evidence](DECISION_CARD_WINDOW_EVIDENCE.md), progress evidence, and native presence evidence are retained | `NO` | Add richer bounded business alternatives without widening approval or dispatch authority, then retain a human-operated cross-application UX result |
