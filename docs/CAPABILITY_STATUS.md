...
| Campaign | `YES` | `PARTIAL` — the synthetic seam and host-status projection remain bounded; two fixed BOSS commands now create the reviewed discovery manifest and dispatch one foreground `ui_snapshot` through Runner/project MCP into stable public job keys | `YES` | `NO` | `YES` — [the exact on-device synthetic path passed](SYNTHETIC_CAMPAIGN_EVIDENCE.md); BOSS page discovery has fake-MCP evidence only | `NO` | Retain one on-device BOSS discovery page, then add reviewed page progression and the multi-item restart gate; no general worker |
