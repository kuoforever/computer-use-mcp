...
`MiniMax-M2.7` China portions of the `PARTIAL` cells above. It remains
model/route-scoped and records the historical Sonnet 5 compatibility failure;
ordinary reasoning preservation and one-shot reasoning normalization each have
exact-commit retained reruns. These records do not alter the offline snapshot
or fill another added-provider, application, or release gate. The separate
