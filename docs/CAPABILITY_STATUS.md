...
| Operator UI | `YES` | `PARTIAL` — console yes/deny approval, fake-only digest-bound Decision Card compilation/selection, passive progress with run/campaign grouping, and an opt-in primary-display presence halo driven by durable ordinary `run`/`resume` phases exist; an interactive card, broader presence wiring, public status tool, and notification bridge do not | `PARTIAL` — [Decision Card evidence](DECISION_CARD_MODEL_EVIDENCE.md) covers fixed trade-offs, estimate provenance, recommendations, safe exits, expiry, every binding drift, and structural absence of authority; progress/presence tests cover redaction, native behavior, and teardown | `NO` | `PARTIAL` — progress has [live polling/campaign evidence](PROGRESS_POLLER_EVIDENCE.md); standalone and integrated presence have [native](PRESENCE_WINDOW_EVIDENCE.md) and [lifecycle](PRESENCE_LIFECYCLE_EVIDENCE.md) evidence; Decision Cards remain offline fake-only | `NO` | Connect a focus-taking local Decision Card to the existing ApprovalPort without a second dispatch path; retain broader runtime evidence before extending presence wiring |
