...
| Operator UI | `YES` | `PARTIAL` — console yes/deny approval, internal fake-host status decisions, passive progress with run/campaign grouping, and a standalone primary-display presence model/controller/Win32 halo exist; automatic presence lifecycle wiring, Decision Cards, public status tool, and notification bridge do not | `PARTIAL` — progress and campaign paths plus 33 presence tests cover structural redaction, click-through/non-activation styles, input-free API, phase/accessibility modes, DPI bounds, capture-affinity reporting, and immediate release/E-stop/terminal teardown | `NO` | `PARTIAL` — progress has [live polling/campaign evidence](PROGRESS_POLLER_EVIDENCE.md); the standalone [presence smoke](PRESENCE_WINDOW_EVIDENCE.md) retained foreground, returned transparent/no-activate native hit tests, retained capture affinity, exercised DPI and reduced motion, and destroyed the halo on E-stop/release | `NO` | Wire presence through one Host-owned lifecycle coordinator, then deliver fake-only Decision Card models; keep mobile delivery host-owned |
