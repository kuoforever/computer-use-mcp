...
| Operator UI | `YES` | `PARTIAL` — console approval, a focus-taking four-choice Decision Card, passive presence/progress, CLI-first Task Center and receipts, Pre-run Review, cooperative control, Approval Inbox/local notifications, and the bounded Windows [accessibility contract](OPERATOR_ACCESSIBILITY.md) exist. Safe keyboard traversal, native UIA Text/Edit/Button semantics, bounded status announcements, system High Contrast/reduced motion, and 200%/400% text reflow are implemented and offline verified with one native UIA/no-activation smoke. General process resume, terminal/mobile notifications, native Inbox UI, complete localization, multi-monitor composition, personalization, and human Narrator/NVDA/live-large-text review remain absent | `YES` — exact approval/re-observe/defer, generated defaults, progress/presence isolation, Task Center receipts, cooperative control, Approval Inbox boundaries, accessibility preference fallback, contrast, focus/order/action, announcements, UIA names, and large-text layout are covered by deterministic tests | `NO` | `YES` — native [Decision Card](DECISION_CARD_WINDOW_EVIDENCE.md), [progress](PROGRESS_LIFECYCLE_EVIDENCE.md), [presence](PRESENCE_WINDOW_EVIDENCE.md), one bounded accessibility UIA smoke, and one human-approved fixed [Chrome-to-Word workflow](PUBLIC_WEB_WORD_PRODUCT_EVIDENCE.md) exist; the accessibility smoke is not retained screen-reader or application evidence | `PARTIAL` — Decision Cards authorized the exact fixed cross-application result; accessibility presentation and passive/read-only/attention surfaces do not widen authority or create application evidence | Run human Narrator/NVDA plus live 200%/400% visual review on the exact feature-freeze candidate, then continue localization/multi-display/personalization as separately scoped slices |
...
| Approval Inbox and local notification | `YES` | `PARTIAL` — Decision Card compilation publishes one strict private expiring identity/digest record, `approval inbox` renders bounded human/JSON inspection without a liveness claim, and an optional Win32 notification carries fixed content only. Inbox and notification have no approval, task-control, provider, MCP, desktop, replay, retry, or dispatch port; console approval, mobile push, localization, and a native Inbox window are excluded | `YES` — binding/expiry lifecycle, registry action coverage, content exclusion, corrupt isolation, strict bounds, fixed notification payload, CLI parity, configuration, notification failure isolation, and Decision Card cleanup are tested | `NO` | `NO` — no retained native notification or assistive-technology result | `NO` — no application behavior is claimed | Run the separate keyboard, screen-reader, high-contrast, and 200%/400% scaling closure before exact-candidate native smoke |
