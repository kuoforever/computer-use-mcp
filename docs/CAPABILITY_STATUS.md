...
1. **Desktop Ask product vertical:** the installed-first-run configuration,
   ...
   public-browser-to-disposable-Word workflow is now functionally fake-verified;
   its exact-candidate provider/Chrome/Word run is the current gate. The
   Notepad result does not broaden the application claim.
