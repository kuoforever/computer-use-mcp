...
| Agent Host | `YES` | `PARTIAL` — installed `config init` / `config doctor`, dual-provider loop, configured optional-tool negotiation, private signed/redacted Claude reasoning-block continuation, locally approved actions, provider request timeout, and fixed `workflow public-web-word` composition. A failed `browser_snapshot` is removed from later turns in the same run. Default `approved_actions` still requires every side effect to be approved; the generated fixed workflow uses Host-owned low/high/unknown classification | `YES` — doctor fail-fast setup UX, exact core-plus-configured-optional MCP discovery, browser-failure withdrawal, stable reviewed Provider contract plus observation-only Runner reacquisition, authorization paths, and wheel resource contract are functional gates | `YES` — [OpenAI and Claude passed](E3_EVIDENCE.md) bounded fake-MCP cases before the optional browser tool; no provider evidence exists for it | `YES` — [both reviewed providers passed](E4_EVIDENCE.md) read-only and one approved action with post-action verification; earlier installed results predate this routing change | `PARTIAL` — one OpenAI Notepad answer and one fixed OpenAI/Chrome/Word workflow passed through the installed product; neither proves optional CDP observation or OS-default ref input | Retain one bounded provider/CDP/Windows result before promoting the new route; keep E4 and release separate |
