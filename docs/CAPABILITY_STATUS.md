...
| Quick Setup and Agent Controls | `YES` | `YES` — `config setup` creates one non-overwriting reviewed default through the strict initializer; `config settings` projects the same TOML as bounded human/JSON purpose, connection, safety, interface, paths, and exact doctor/shortcut-host commands. Credentials remain environment-only; the settings projection has no authority or registration ownership | `YES` — defaults/overrides, non-overwrite, secret exclusion, strict loading, provider-presence booleans, human/JSON parity, and CLI routing are deterministic tests | `NO` | `NO` — the settings surface opens no provider, MCP, application, or desktop port | `NO` | Preserve the inert settings/liveness boundary; widen only through a separately evidenced surface |
| ShortcutBroker | `YES` | `YES` — explicit `shortcuts run` atomically registers fixed `Ctrl+Alt+G` presentation and `Ctrl+Alt+P` cooperative-pause request with Win32 `RegisterHotKey + MOD_NOREPEAT`; ACTIVE follows successful G/P/timer startup, conflicts roll back, exit unregisters, and only exact `paused + authority=released` is safe. The independent Q emergency stop is not registered; global approve/resume do not exist | `YES` — G/P/no-Q registration, conflict rollback, cleanup, presentation failure, strict config refresh, request-versus-release state, unavailable/drift handling, and CLI/service composition are deterministic tests | `NO` | `PARTIAL` — one [Windows 11 non-input native run](SHORTCUT_BROKER_WINDOWS_EVIDENCE.md) passed real G/P registration/message routing, cross-process conflict, atomic rollback, release/reacquisition, unchanged foreground, and no G/P AltGr mapping on the currently loaded `zh-CN`/`en-US` layouts. Physical-key triggering and other layouts remain unverified | `NO` | Preserve the fixed-key result; configurable shortcuts require a separate contract and physical/other-layout evidence remains separate |
