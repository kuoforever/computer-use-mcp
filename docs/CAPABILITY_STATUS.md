...
| Operator UI | `YES` | `PARTIAL` — console yes/deny approval, internal fake-host status decisions, and the pure checkpoint-to-view-model reducer exist; presence indicator, passive progress window, Decision Cards, public status tool, and notification bridge do not | `PARTIAL` — approval, status-decision, and progress-reducer paths have offline evidence | `NO` | `NO` | `NO` | Draw the passive non-activating viewer over the reducer and add its isolated smoke; keep mobile delivery host-owned |
