...
| Campaign | `YES` | `PARTIAL` — the synthetic seam and host-status projection remain bounded; fixed BOSS commands accumulate stable public keys, start the coordinator-selected batch, verify and digest-commit one exact claimed identity through one Runner/project-MCP snapshot, write handoff, then transfer ownership to a fresh zero-port run and claim the exact next item | `YES` — discovery refusal/reconstruction, exact-plan start/claim, identity-drift refusal, single-call commit/handoff, fresh/stale owner transfer/resume, trace privacy, and zero-selector CLI behavior are tested | `NO` | `PARTIAL` — the [synthetic path](SYNTHETIC_CAMPAIGN_EVIDENCE.md), [two-pass discovery](BOSS_CAMPAIGN_MULTIPAGE_EVIDENCE.md), and a partial [three-item restart diagnostic](BOSS_ITEM_RESTART_DIAGNOSTIC_EVIDENCE.md) passed on-device; the diagnostic exposed and retained two corrected integration defects | `PARTIAL` — three identity-only commits and one clean post-fix stale recovery, not pristine acceptance, semantic extraction, provider rotation, or a multi-item gate | Run a fresh uncorrected bounded multi-item sequence on the fixed code; no general worker |
...
| Operator UI | `YES` | `PARTIAL` — console approval, an opt-in four-choice focus-taking Decision Card with configurable corner placement, normal drag/resize/minimize/maximize behavior, non-topmost stacking, responsive buttons, scrollable digest evidence, same-run re-observe, durable non-resumable defer, passive progress with explicit token-coverage, screenshot, and checkpoint-elapsed facts, an opt-in fail-silent ordinary-run, bounded-plan, read-only recovery, and fixed MCP-backed campaign execution progress lifecycle on a dedicated UI thread, and opt-in durable ordinary-run/bounded-plan presence exist; zero-port campaign control remains window-free, while recovery/campaign presence wiring, public status, notifications, and general process resume do not | `YES` — [re-observe/defer evidence](DECISION_CARD_RECOVERY_EVIDENCE.md) covers zero-dispatch decisions, stale-turn abandonment, fresh-observation gating, `PAUSED` persistence/recovery projection, and the sole Runner dispatch boundary; progress/presence tests cover redaction, legacy unknowns, metric integrity, lifecycle isolation, durable bounded-plan projection, phase-free campaign wake, and teardown | `NO` | `YES` — native [four-option Decision Card focus/resize/scroll/timeout evidence](DECISION_CARD_WINDOW_EVIDENCE.md), ordinary progress [lifecycle evidence](PROGRESS_LIFECYCLE_EVIDENCE.md), provider-free bounded-plan [progress evidence](PLAN_PROGRESS_LIFECYCLE_EVIDENCE.md), persisted read-only [recovery progress evidence](RECOVERY_PROGRESS_LIFECYCLE_EVIDENCE.md), fixed synthetic [campaign progress evidence](CAMPAIGN_PROGRESS_LIFECYCLE_EVIDENCE.md), and ordinary-run native presence evidence are retained; bounded-plan presence and BOSS campaign progress wiring remain offline-only | `NO` | Retain a human-operated four-choice cross-application UX result without widening action authority |
