...
| Operator UI | `YES` | `PARTIAL` — console approval, an opt-in four-choice focus-taking Decision Card with configurable corner placement, normal drag/resize/minimize/maximize behavior, non-topmost stacking, responsive buttons, scrollable digest evidence, same-run re-observe, durable non-resumable defer, passive progress with explicit token-coverage, screenshot, and checkpoint-elapsed facts, an opt-in fail-silent ordinary-run progress lifecycle on a dedicated UI thread, and opt-in durable ordinary-run presence exist; plan/campaign/recovery UI wiring, broader presence wiring, public status, notifications, and general process resume do not | `YES` — [re-observe/defer evidence](DECISION_CARD_RECOVERY_EVIDENCE.md) covers zero-dispatch decisions, stale-turn abandonment, fresh-observation gating, `PAUSED` persistence/recovery projection, and the sole Runner dispatch boundary; progress/presence tests cover redaction, legacy unknowns, metric integrity, lifecycle isolation, and teardown | `NO` | `YES` — native [four-option Decision Card focus/resize/scroll/timeout evidence](DECISION_CARD_WINDOW_EVIDENCE.md), progress [lifecycle evidence](PROGRESS_LIFECYCLE_EVIDENCE.md), and native presence evidence are retained | `NO` | Retain a human-operated four-choice cross-application UX result without widening action authority |
