...
| Operator UI | `YES` | `PARTIAL` — console approval, a focus-taking four-choice Decision Card, passive presence/progress, CLI-first Task Center and receipts, Pre-run Review, cooperative control, and the separate Approval Inbox/local notification slice exist within bounded workflows. Newly generated installed profiles explicitly enable action feedback plus all six `[operator]` UI/UX booleans, including fixed-content approval notifications; legacy/manual absent-key defaults remain compatible. General process resume, terminal/mobile notifications, native Inbox UI, complete localization, multi-monitor composition, and accessibility closure remain absent | `YES` — exact approval/re-observe/defer, generated defaults, progress/presence isolation, Task Center receipts, cooperative control, and Approval Inbox content/authority boundaries are covered by deterministic tests | `NO` | `YES` — native [Decision Card](DECISION_CARD_WINDOW_EVIDENCE.md), [progress](PROGRESS_LIFECYCLE_EVIDENCE.md), [presence](PRESENCE_WINDOW_EVIDENCE.md), and one human-approved fixed [Chrome-to-Word workflow](PUBLIC_WEB_WORD_PRODUCT_EVIDENCE.md) are retained; Task Center, cooperative control, and Approval Inbox notifications have no retained native result | `PARTIAL` — Decision Cards authorized the exact fixed cross-application result; passive/read-only/default/attention surfaces do not widen authority or create new application evidence | Close keyboard, screen-reader, high-contrast, reduced-motion, and 200%/400% scaling gaps before the exact feature-freeze candidate |
