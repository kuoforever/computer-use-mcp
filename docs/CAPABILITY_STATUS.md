...
| Campaign | `YES` | `PARTIAL` — in addition to the retained identity/restart seam, three fixed semantic CLIs open one-item/five-call/zero-side-effect batches, re-establish the exact claim through Runner, accept only strict provider JSON under a fixed no-preference policy, commit canonical result digests, write terminal handoff, and transfer a successful batch to a fresh run | `YES` — discovery/refusal/reconstruction, both batch policies, identity drift, strict result/source/policy binding, UIA/document escalation, challenge and unmet-OCR-baseline handoff, successful fresh-run transfer, trace privacy, and zero-selector CLI behavior are tested in the [semantic contract](BOSS_SEMANTIC_EXTRACTION_CONTRACT.md) | `NO` | `PARTIAL` — the [synthetic path](SYNTHETIC_CAMPAIGN_EVIDENCE.md), [two-pass discovery](BOSS_CAMPAIGN_MULTIPAGE_EVIDENCE.md), historical [three-item diagnostic](BOSS_ITEM_RESTART_DIAGNOSTIC_EVIDENCE.md), and clean [three-item restart sequence](BOSS_ITEM_RESTART_CLEAN_EVIDENCE.md) passed on-device | `PARTIAL` — identity/restart passed, but no semantic item, provider rotation, or 100-item gate has passed | Retain one on-device UIA/document-text semantic item; no general worker |
| Observation | `YES` | `PARTIAL` — UIA, full primary-display screenshot, bounded region OCR, bounded region image capture, bounded UIA document text, bounded Agent image handling, a pure full BOSS ladder reducer, and a runtime UIA/document subset exist; delta observations do not | `YES` — strict ladder escalation/handoff and runtime zero-dispatch behavior for the still-gated OCR Host baseline are tested alongside source limits, schemas, redaction, timeout, and result projection | `NO` | `PARTIAL` — [BOSS OCR evidence](BOSS_OCR_EVIDENCE.md) recovered a missing static tab and matched one job card to UIA; bounded [document text](DOCUMENT_TEXT_EVIDENCE.md) and a synthetic [region capture](CAPTURE_REGION_EVIDENCE.md) passed on-device | `PARTIAL` — no semantic item has passed through the new runtime | Retain UIA/document semantics, then review the OCR Host safety baseline separately |
