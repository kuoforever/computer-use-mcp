...
> **Status: current review dashboard, verified 2026-08-23.** This page is the
...
The planned [Formal Demo v1](FORMAL_DEMO_V1.md) does not exist as an executable
...
live provider intent adapter/call, launcher,
executable adapters, durable run,
GitHub/PDF/Excel/Word/email-draft composition, Formal Demo Receipt, and formal
evidence remain absent. The built-in email role is explicitly `UNSELECTED`, so
the complete product scenario currently fails closed. In the Operator UI row,
"console" means the existing terminal/CLI approval surface, not that planned
Agent Console. The implemented `public-web-word` Pre-run Review remains a
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Formal Demo v1 | `YES` — the selected role-based product story and safety boundary are specified in [Formal Demo v1](FORMAL_DEMO_V1.md) | `PARTIAL` — internal inert v1 intent/scenario/profile/Scope contracts, exact registry pins, fail-closed compilation, a typed local disclosure/exact-`COMPILE` permit gate, and a provider-neutral injected-port one-attempt coordinator exist; no Console, concrete provider adapter/call, launcher, executable adapter, durable composition, or Receipt exists | `PARTIAL` — the 137-test focused `GDA-DEMO-007A`/`007B`/`007C` gate covers contract binding plus exact disclosure/permit issue-consume, descriptor-safe consume-before-fake ordering, forced-overlap one-call concurrency, sanitized cancellation/error handling, refusal, truncation, malformed/oversized output, strict post-construction revalidation, reviewed-scenario pin/drift/expansion rejection, and no-wiring behavior. This is deterministic offline evidence only; it is not process-wide or crash-safe exactly-once and proves no provider call | `NO` | `NO` | `NO` | Do not activate from this dashboard. No row is active; a no-key Review-only Agent Console is only a possible separately activated slice. Live provider/E3 work remains deferred and would need current route/account/data-control authorization. The email adapter remains unselected, and no fixed-workflow, application-coverage, or Universal-showcase evidence transfers |
