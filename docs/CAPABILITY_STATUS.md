...
| Hierarchical control | `YES` — [the closed node set, fail-closed propagation, `H1`-`H8` order, and uncertainty boundary](HIERARCHICAL_TASK_AND_BEHAVIOR_TREES.md) are fixed | `PARTIAL` — H1-H3 add immutable versioned nodes, canonical tree/envelope digests, structural/budget limits, pure total parent reduction, lossless linear-plan projection, private `RunLock`-bound exact-CAS snapshots, and pure next-leaf/ordered-transition compilation; there is no Runner, provider, MCP, desktop, or authority port | `YES` — topology, limits, digest binding, malformed contracts, linear compatibility, registry/restart drift, stale writes, every pre-commit persistence failure, frozen next-leaf traces, re-emission prevention, ordering, active-state drift, and unresolved choices fail closed in deterministic tests | `N/A` | `N/A` | `N/A` | Implement H4 observation-only tree wiring through the sole existing Runner boundary |
...
1. **Hierarchical control and verified learning:** H1-H3 and L0 now fix the inert node,
   ...
   contracts. H4 observation-only Runner wiring is exact next. None of these early phases
