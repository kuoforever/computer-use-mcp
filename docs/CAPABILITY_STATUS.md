...
| Operator UI | `YES` | `PARTIAL` — console yes/deny approval, internal fake-host status decisions, the checkpoint-to-view-model reducer, passive non-activating progress window, atomic live polling, and fixed independent-run grouping exist; presence indicator, campaign grouping, Decision Cards, public status tool, and notification bridge do not | `PARTIAL` — approval, status-decision, progress-reducer, passive-window (foreground-unchanged in injectable form; structural redaction), live-polling (change-only redraw, stale-view discard, isolated corrupt record, [concurrent publish/read contract](CHECKPOINT_PUBLISH_EVIDENCE.md)), and multi-run grouping (strict timestamps, stable ordering, attention priority, duplicate refusal, regrouping) paths have offline evidence | `NO` | `PARTIAL` — the [passive-window non-activation smoke](PROGRESS_WINDOW_EVIDENCE.md) drew, refreshed, moved, and re-topped the real window over synthetic records with the foreground HWND unchanged, and the [live-polling smoke](PROGRESS_POLLER_EVIDENCE.md) followed a real `RunRecorder` transition to the drawn window with the foreground unchanged and 400/400 concurrent publishes succeeding; both predate grouping and neither covers campaigns or a long soak | `NO` | Deliver campaign progress (step 5), then presence; keep mobile delivery host-owned |
