...
> **Status: current review dashboard, verified 2026-08-10.** This page is the
...
| Agent Host | `YES` | `PARTIAL` — installed setup/doctor and ordinary/Planner/final routing cover eight exact profiles across three wire families plus an allowlisted service-region catalog. Provider name, region-specific credential, endpoint, protocol, capabilities, continuation v8 identity, and image-tool surface remain separate. Locally approved actions, timeout, optional-tool withdrawal, and fixed `workflow public-web-word` still use the sole Runner/MCP authority | `YES` — catalog/factory regional routing, fixed versus constructed Qwen workspace endpoints, regional credential isolation, three-family request/continuation contracts, v8 cross-region recovery refusal, strict v7/v6 read compatibility, prompt-only structured-output validation, text-only image withdrawal, doctor/setup, and existing Runner authorization paths are deterministic gates | `PARTIAL` — only the earlier [OpenAI and Claude scopes passed](E3_EVIDENCE.md); no added provider or regional route made a real API request | `PARTIAL` — only the two earlier reviewed providers have [retained E4 evidence](E4_EVIDENCE.md), predating the added profiles and current routing | `PARTIAL` — one OpenAI Notepad answer and one fixed OpenAI/Chrome/Word workflow are retained; no added provider has application evidence | After regional accounts exist, run exact-provider/model/region harmless fake-MCP E3 one route at a time; separately authorize E4/application gates and never promote sibling providers or regions |
| Planner / Executor | `YES` | `PARTIAL` — `ask` / `plan run` and `workflow public-web-word` route every provider profile and reviewed region through one tool-free Planner and final-response factory. Native JSON Schema, JSON object, and exact-schema prompt modes remain explicit; all output is compiled and validated as untrusted data | `YES` — all provider and regional routes, provider-specific request fields, tool-free one-call behavior, byte/token limits, strict schema compilation, image-capability failures, and no cross-region fallback are covered with fake clients; ordinary Runtime composition remains unchanged | `PARTIAL` — [OpenAI and Claude passed](E3_EVIDENCE.md) the earlier bounded fake-MCP scope, and same-wheel OpenAI `gpt-5.6-terra` [Notepad and fixed Chrome-to-Word plans](CURRENT_CANDIDATE_PRODUCT_INTEGRATION_EVIDENCE.md) passed; six added profiles and all newly added region variants are live-unverified | `PARTIAL` — one Windows 11 foreground `document_text -> final_response` plan and one fixed model-driven Chrome-to-Word plan passed from one clean wheel, both under earlier OpenAI scope | `PARTIAL` — one synthetic Notepad answer and one fixed public-browser-to-disposable-Word workflow are retained | Run exact Planner/final/tool/image/continuation E3 cells for each provider/model/region route before any live claim |
...
| Quick Setup and Agent Controls | `YES` | `YES` — `config setup` / `config init` accept all eight exact provider names, typed reviewed regions, and Qwen workspace IDs; `config settings` projects the same strict TOML, effective region, regional SDK/key-presence booleans, shortcuts, paths, and exact doctor/shortcut-host commands. Credentials remain environment-only; arbitrary endpoints are rejected and the settings projection has no authority or registration ownership | `YES` — eight-provider defaults, regional overrides, Qwen URL construction and strict legacy migration, fixed-endpoint rejection, pause validation, non-overwrite, secret exclusion, strict loading, presence booleans, human/JSON parity, and CLI routing are deterministic tests | `NO` — setup and settings make no provider request | `NO` — the settings surface opens no provider, MCP, application, or desktop port | `NO` | Preserve the inert settings/liveness boundary; live readiness starts only at an explicitly run provider/model/region gate |
...
Separately, the retained [provider E3 record](E3_EVIDENCE.md) supports only the
historical OpenAI/Claude portions of the `PARTIAL` cells above. It is model-scoped and records the historical
...
or fills any added-provider, application, or release gate. The separate
...
1. **Hierarchical control and verified learning:** H1-H8 and L0-L4 now fix the node,
   ...
   choice/read-only verified-miss fallback. H8C publication is exact next; L5
   still requires separate consent. None of these phases
