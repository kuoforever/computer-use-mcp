...
| Hierarchical control | `YES` — [the closed node set, fail-closed propagation, `H1`-`H8` order, and uncertainty boundary](HIERARCHICAL_TASK_AND_BEHAVIOR_TREES.md) are fixed | `PARTIAL` — H1-H7 add immutable nodes, exact digests/CAS, pure next-leaf compilation, typed fresh facts, one exact BOSS observation template, and one separately reviewed observation/action/verification-observation/final sequence. H8A adds contract-v2 bounded parallel evaluation of 2-16 direct H5 conditions with at most four local workers and one atomic known-result CAS. H7 still reuses the existing Runtime Executor plus sole Runner boundary; H8A has no external boundary, and H4/public Planner/Executor remain observation-only | `YES` — prior H1-H7 gates plus H8A actual worker overlap/four-worker ceiling, deterministic same-snapshot binding, v1/v2 digest/decode compatibility, false/unavailable precedence, exception/CAS zero-partial-commit, strict tamper rejection, and restart semantics pass deterministically | `N/A` | `PARTIAL` — [H7 isolated-application evidence](H7_BOUNDED_SIDE_EFFECT_EVIDENCE.md) uses production stores/compiler/Runner/ledger with injected desktop and approval ports. [H8A evidence](H8A_PARALLEL_CONDITION_EVIDENCE.md) is port-free source/offline evidence only; no real MCP, Windows desktop, provider, external application, or external parallelism is claimed | `N/A` | Publish H8A cleanly, then implement bounded all-of DAG/join as H8B without widening the sole Runner boundary |
...
1. **Hierarchical control and verified learning:** H1-H7, H8A, and L0-L4 now fix the node,
   ...
   route binding, and bounded local parallel H5 condition batches. H8B is exact
   next only after clear H8A publication. None of these phases
