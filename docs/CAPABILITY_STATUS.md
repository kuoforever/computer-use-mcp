...
and fail-closed structural compilation. The independent Agent Console,
provider intent call, launcher, executable adapters, durable run,
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Formal Demo v1 | `YES` — the selected role-based product story and safety boundary are specified in [Formal Demo v1](FORMAL_DEMO_V1.md) | `PARTIAL` — internal inert v1 intent/scenario/profile/Scope contracts, exact registry pins, and fail-closed compilation exist; no Console, provider compiler/call, launcher, executable adapter, durable composition, or Receipt exists | `PARTIAL` — `GDA-DEMO-007A` contract round-trip, canonical digest, Unicode/size bounds, tamper, authority, reviewed-pin, unavailable-profile, and no-port behavior are deterministic tests only | `NO` | `NO` | `NO` | Do not activate from this dashboard. Select exactly one later slice only through [Project status](../PROJECT_STATUS.md); the email adapter remains unselected, and no fixed-workflow, application-coverage, or Universal-showcase evidence transfers |
