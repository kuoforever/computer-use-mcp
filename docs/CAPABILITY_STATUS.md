...
| Hierarchical control | `YES` — [the closed node set, fail-closed propagation, `H1`-`H8` order, and uncertainty boundary](HIERARCHICAL_TASK_AND_BEHAVIOR_TREES.md) are fixed | `PARTIAL` — H1-H7 add immutable nodes, exact digests/CAS, pure next-leaf compilation, typed fresh facts, one exact BOSS observation template, and one separately reviewed observation/action/verification-observation/final sequence. H8A adds contract-v2 bounded parallel evaluation of 2-16 direct H5 conditions; H8B adds contract-v3 bounded all-of dependencies, general parallel subtrees, stable one-ready-leaf selection, and local joins. H7 still reuses the existing Runtime Executor plus sole Runner boundary; H8A-H8B add no external port, and H4/public Planner/Executor remain observation-only | `YES` — prior H1-H8A gates plus H8B missing/self/duplicate/cycle/fan-in/edge/depth rejection, join state matrix, stable ready-leaf order, global external serialization, v1-v3 digest/decode compatibility, strict tamper rejection, and restart semantics pass deterministically | `N/A` | `PARTIAL` — [H7 isolated-application evidence](H7_BOUNDED_SIDE_EFFECT_EVIDENCE.md) uses production stores/compiler/Runner/ledger with injected desktop and approval ports. [H8A](H8A_PARALLEL_CONDITION_EVIDENCE.md) and [H8B evidence](H8B_DEPENDENCY_JOIN_EVIDENCE.md) are port-free source/offline evidence only; no real MCP, Windows desktop, provider, external application, or external parallelism is claimed | `N/A` | Publish H8B cleanly, then implement safe ordered choice and exact read-only verified-miss fallback as H8C |
...
1. **Hierarchical control and verified learning:** H1-H7, H8A-H8B, and L0-L4 now fix the node,
   ...
   local joins, and deterministic one-ready-leaf selection. H8C is exact next
   only after clear H8B publication. None of these phases
