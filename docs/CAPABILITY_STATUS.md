...
loader, command, persistence, or execution port. The live provider intent
adapter/call, executable adapters, durable run, GitHub/PDF/Excel/Word/email-draft
composition, Formal Demo Receipt, and formal evidence remain absent.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Formal Demo v1 | `YES` — the selected role-based product story and safety boundary are specified in [Formal Demo v1](FORMAL_DEMO_V1.md) | `PARTIAL` — internal inert v1 intent/scenario/profile/Scope contracts, exact registry pins, fail-closed compilation, a typed local disclosure/exact-`COMPILE` permit gate, a provider-neutral injected-port one-attempt coordinator, an explicit Host-owned fixed local compiler, five selected inert role pins, and an independent no-key Offline Scope Review Windows Console/launcher exist. The Console reaches a complete reviewed Scope and stops with native `Start` disabled; no concrete provider adapter/call, executable application adapter, durable composition, or Receipt exists | `PARTIAL` — the `GDA-DEMO-007A`/`007B`/`007C` gates cover contract binding, disclosure, permit, and injected-fake one-attempt behavior. The `007D`/`007E` gates cover exact local text, fixed local mapping, registry-pin and digest binding, one-success concurrent permit consumption, complete Scope rendering, zero-provider/zero-retry budgets, fixed failures, no-key/no-wiring launcher behavior, real hidden Win32 disabled/inert `Start`, keyboard routing, work-area/reflow, resize/minimize, lifecycle, and base-wheel startup without provider extras. This is deterministic offline/native-component evidence only; it is not process-wide or crash-safe exactly-once and proves no provider, Desktop E4, application, or human-accessibility result | `NO` | `NO` | `NO` | Stop at the implemented Offline Scope Review boundary. Live provider/E3, `START`, executable role adapters, durable composition, and Receipt remain deferred and require separately explicit scope and authority. Outlook Desktop is only a selected inert design target, and no fixed-workflow, application-coverage, or Universal-showcase evidence transfers |
