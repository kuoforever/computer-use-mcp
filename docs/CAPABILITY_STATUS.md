...
| Hierarchical control | `YES` — [the closed node set, fail-closed propagation, `H1`-`H8` order, and uncertainty boundary](HIERARCHICAL_TASK_AND_BEHAVIOR_TREES.md) are fixed | `PARTIAL` — H1-H4 add immutable versioned nodes, canonical tree/envelope digests, structural/budget limits, pure total parent reduction, lossless linear-plan projection, private `RunLock`-bound exact-CAS snapshots, pure next-leaf compilation, and an optional observation/final-response status projection over the existing Runtime Executor. It adds no second Runner/MCP dispatch, side effects, retry, replay, or recovery authority | `YES` — topology, limits, digest binding, malformed contracts, linear compatibility, registry/restart drift, stale writes, every pre-commit persistence failure, frozen next-leaf traces, exact pre-boundary activation, correlated known results, uncertainty retention, zero-dispatch failures, local-only repair, final completion, re-emission prevention, ordering, active-state drift, and unresolved choices fail closed in deterministic tests | `N/A` | `N/A` — H4 evidence uses injected fake ports only | `N/A` | Implement H5 typed world-state facts and freshness invalidation; no condition may consume stale observations |
...
1. **Hierarchical control and verified learning:** H1-H4 and L0 now fix the node,
   ...
   contracts plus observation-only Runtime Executor composition. H5 typed
   world-state facts and freshness invalidation are exact next. None of these
   early phases adds side-effect authority or automatic learning.
