...
| Hierarchical control | `YES` — [the closed node set, fail-closed propagation, `H1`-`H8` order, and uncertainty boundary](HIERARCHICAL_TASK_AND_BEHAVIOR_TREES.md) are fixed | `PARTIAL` — H1-H5 add immutable versioned nodes, canonical tree/envelope digests, structural/budget limits, pure total parent reduction, lossless linear-plan projection, private `RunLock`-bound exact-CAS snapshots, pure next-leaf compilation, an optional observation/final-response status projection over the existing Runtime Executor, and typed content-free observation facts with exact run/epoch/generation/window/time binding and three-valued conditions. They add no second Runner/MCP dispatch, side effects, retry, replay, recovery, or learning authority | `YES` — topology, limits, digest binding, malformed contracts, linear compatibility, registry/restart drift, stale writes, persistence failures, frozen next-leaf traces, exact pre-boundary activation, correlated known results, uncertainty retention, zero-dispatch failures, local-only repair, final completion, typed value validation, raw-content exclusion, deterministic fact/snapshot digests, empty/missing/unknown facts, epoch/generation/window/type/time invalidation, unavailable-not-false conditions, ordering, active-state drift, and unresolved choices fail closed in deterministic tests | `N/A` | `N/A` — H4-H5 evidence uses injected data/fake ports only | `N/A` | Implement H6 pinned reviewed behavior templates beginning with the per-item observation ladder; H5 facts still cannot select or execute a branch alone |
| Continual Learning | `YES` | `PARTIAL` — L0 derives a versioned redacted terminal outcome and fixed explicit-coverage run cost vector from the existing validated Full Cycle record, optionally reconciling one exact locked campaign item; it writes nothing and cannot extract, inject, score, route, promote, train, or execute | `YES` — terminal-only labels, uncertain/challenge/conflict precedence, trace/checkpoint/budget metric reconciliation, complete/partial/missing coverage, campaign source binding, privacy exclusion, deterministic digests, and filesystem non-mutation fail closed in deterministic tests | `N/A` | `N/A` | `N/A` | Preserve L0 while H4-H6 mature; L1 quarantine extraction remains later and cannot auto-inject |
...
1. **Hierarchical control and verified learning:** H1-H5 and L0 now fix the node,
   ...
   contracts plus observation-only Runtime Executor composition and typed
   freshness/window-bound facts. H6 pinned reviewed behavior templates are
   exact next. None of these early phases adds side-effect authority or
   automatic learning.
