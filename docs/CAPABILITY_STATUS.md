...
| Agent Host | `YES` | `PARTIAL` — installed setup/doctor and ordinary/Planner/final routing now cover eight exact profiles: OpenAI, Anthropic, Qwen, Doubao, Kimi, DeepSeek, GLM, and MiniMax across three wire families. Provider name, credential, endpoint, protocol, capabilities, continuation v7 identity, and image-tool surface remain separate. Locally approved actions, timeout, optional-tool withdrawal, and fixed `workflow public-web-word` still use the sole Runner/MCP authority | `YES` — catalog/factory routing, fixed versus Qwen workspace endpoint validation, credential isolation, three-family request/continuation contracts, prompt-only structured-output validation, text-only image withdrawal, doctor/setup, recovery identity, and existing Runner authorization paths are deterministic gates | `PARTIAL` — only the earlier [OpenAI and Claude scopes passed](E3_EVIDENCE.md); Kimi/Qwen/Doubao/DeepSeek/GLM/MiniMax made no real API request | `PARTIAL` — only the two earlier reviewed providers have [retained E4 evidence](E4_EVIDENCE.md), predating the added profiles and current routing | `PARTIAL` — one OpenAI Notepad answer and one fixed OpenAI/Chrome/Word workflow are retained; no added provider has application evidence | After accounts exist, run exact-provider/model harmless fake-MCP E3 one profile at a time; separately authorize E4/application gates and never promote sibling providers |
| Planner / Executor | `YES` | `PARTIAL` — `ask` / `plan run` and `workflow public-web-word` route every provider profile through one tool-free Planner and final-response factory. Native JSON Schema, JSON object, and exact-schema prompt modes remain explicit; all output is compiled and validated as untrusted data | `YES` — all eight routes, provider-specific request fields, tool-free one-call behavior, byte/token limits, strict schema compilation, image-capability failures, and no fallback are covered with fake clients; ordinary Runtime composition remains unchanged | `PARTIAL` — [OpenAI and Claude passed](E3_EVIDENCE.md) the earlier bounded fake-MCP scope, and same-wheel OpenAI `gpt-5.6-terra` [Notepad and fixed Chrome-to-Word plans](CURRENT_CANDIDATE_PRODUCT_INTEGRATION_EVIDENCE.md) passed; six added profiles are live-unverified | `PARTIAL` — one Windows 11 foreground `document_text -> final_response` plan and one fixed model-driven Chrome-to-Word plan passed from one clean wheel, both under earlier OpenAI scope | `PARTIAL` — one synthetic Notepad answer and one fixed public-browser-to-disposable-Word workflow are retained | Run exact Planner/final/tool/image/continuation E3 cells for each added provider before any live claim |
...
| Quick Setup and Agent Controls | `YES` | `YES` — `config setup` / `config init` accept all eight exact provider names and one Qwen-only reviewed workspace endpoint; `config settings` projects the same strict TOML, provider SDK/key-presence booleans, effective shortcuts, paths, and exact doctor/shortcut-host commands. Credentials remain environment-only; fixed providers reject endpoint overrides and the settings projection has no authority or registration ownership | `YES` — eight-provider defaults/overrides, Qwen endpoint allowlisting, fixed-endpoint rejection, pause validation, non-overwrite, secret exclusion, strict loading, presence booleans, human/JSON parity, and CLI routing are deterministic tests | `NO` — setup and settings make no provider request | `NO` — the settings surface opens no provider, MCP, application, or desktop port | `NO` | Preserve the inert settings/liveness boundary; live readiness starts only at an explicitly run provider gate |
