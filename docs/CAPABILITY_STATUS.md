...
| Hierarchical control | `YES` — [the closed node set, fail-closed propagation, `H1`-`H8` order, and uncertainty boundary](HIERARCHICAL_TASK_AND_BEHAVIOR_TREES.md) are fixed | `PARTIAL` — H1-H2 add immutable versioned nodes, canonical tree/envelope digests, reviewed structural/budget limits, pure total parent reduction, lossless linear-plan projection, and private `RunLock`-bound atomic snapshots with exact sequence/tree-digest CAS; there is no next-leaf compiler, Runner, provider, MCP, desktop, or authority port | `YES` — topology, limits, budgets, digest binding, every status combination, malformed contracts, linear-plan compatibility, registry/restart drift, stale writes, and every pre-commit persistence failure fail closed in deterministic tests | `N/A` | `N/A` | `N/A` | Implement H3 pure next-leaf compilation and frozen offline trace fixtures; emit at most one boundary per tick and never dispatch |
...
1. **Hierarchical control and verified learning:** H1-H2 now fix the inert node,
   digest, limit, reduction, linear-plan compatibility, and private atomic
   persistence contracts. H3 pure next-leaf compilation is exact next, then L0
