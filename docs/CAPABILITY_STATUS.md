...
| Quick Setup and Agent Controls | `YES` | `YES` — `config setup` creates one non-overwriting reviewed default through the strict initializer; `config settings` projects the same TOML as bounded human/JSON purpose, connection, safety, interface, paths, and exact doctor command. Credentials remain environment-only; all authority flags and shortcut registration are false | `YES` — defaults/overrides, non-overwrite, secret exclusion, strict loading, provider-presence booleans, human/JSON parity, and CLI routing are deterministic tests | `NO` | `NO` — the surface opens no provider, MCP, application, or desktop port | `NO` | Add the separately bounded ShortcutBroker for presentation-only open-controls and safe-pause requests; never add global approve or resume |
