...
| Observation | `YES` | `PARTIAL` — UIA, full primary-display screenshot, bounded region OCR, bounded region image capture, and bounded Agent image handling exist; document text and delta sources do not | `YES` — OCR and capture limits, schemas, redaction, timeout, and result projection are tested | `NO` | `PARTIAL` — [BOSS OCR evidence](BOSS_OCR_EVIDENCE.md) recovered a missing static tab and matched one job card to UIA; region capture has no on-device result | `PARTIAL` — one real page and card, not application acceptance | Retain an on-device region-capture result, then exercise the ladder across bounded multi-item and restart cases |
| Operator UI | `YES` | `PARTIAL` — console yes/deny approval, internal fake-host status decisions, the checkpoint-to-view-model reducer, the passive non-activating progress window, and atomic live checkpoint polling over it exist; presence indicator, multi-run grouping, Decision Cards, public status tool, and notification bridge do not | `PARTIAL` — approval, status-decision, progress-reducer, passive-window (foreground-unchanged in injectable form; structural redaction), and live-polling (change-only redraw, stale-view discard, isolated corrupt record, [concurrent publish/read contract](CHECKPOINT_PUBLISH_EVIDENCE.md)) paths have offline evidence | `NO` | `PARTIAL` — the [passive-window non-activation smoke](PROGRESS_WINDOW_EVIDENCE.md) drew, refreshed, moved, and re-topped the real window over synthetic records with the foreground HWND unchanged, and the [live-polling smoke](PROGRESS_POLLER_EVIDENCE.md) followed a real `RunRecorder` transition to the drawn window with the foreground unchanged and 400/400 concurrent publishes succeeding; neither covers grouping, campaigns, or a long soak | `NO` | Deliver multi-run grouping (step 4), then campaign progress; keep mobile delivery host-owned |
