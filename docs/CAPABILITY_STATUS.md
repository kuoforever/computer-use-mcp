...
| Planner / Executor | `YES` | `PARTIAL` — declarative planning, observation/final runtime, and local observation/final reconciliation are internal only | `YES` | `NO` | `NO` | `NO` | Expose one bounded observation-only CLI path without adding a second dispatch boundary |
...
~~~text
python -m pytest -q              867 passed, 3 skipped
