...
1. **Hierarchical control and verified learning:** H1-H3 now fix the inert node,
   digest, limit, reduction, linear-plan compatibility, private atomic
   persistence, and pure next-leaf contracts. L0 normalized episode outcome
   and cost instrumentation is exact next. None of these early phases
