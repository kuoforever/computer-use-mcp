...
| --- | --- | --- | --- | --- | --- | --- | --- |
| MCP Server | `YES` | `YES` — thirteen-tool Windows stdio server, including bounded OCR, region capture, UIA document text, scroll, and drag | `YES` | `N/A` | `PARTIAL` — [the five-case Windows activation regression](E4_EVIDENCE.md), bounded [document-text result](DOCUMENT_TEXT_EVIDENCE.md), and exact-candidate [Desktop Ask result](DESKTOP_ASK_EVIDENCE.md) passed on-device; scroll and drag are offline-verified only | `PARTIAL` — bounded BOSS [home](BOSS_EVIDENCE.md) and [interested-jobs OCR](BOSS_OCR_EVIDENCE.md) observations plus real UIA [document text](DOCUMENT_TEXT_EVIDENCE.md) and one Notepad [Desktop Ask](DESKTOP_ASK_EVIDENCE.md) passed; no multi-step product application workflow has passed | Retain a model-driven public-browser-to-disposable-Word workflow, then broaden applications only through retained evidence |
| Agent Host | `YES` | `PARTIAL` — installed-first-run `config init` / `config doctor`, dual-provider read-only loop, private signed/redacted Claude reasoning-block continuation, and locally approved actions | `YES` — doctor fail-fast setup UX and exact thirteen-tool installed-MCP discovery are fake-verified; the wheel path is a CI gate | `YES` — [OpenAI and Claude passed](E3_EVIDENCE.md) both bounded fake-MCP E3 cases with reviewed model IDs; Sonnet 5 compatibility is revalidated on the exact repair commit | `YES` — [both reviewed providers passed](E4_EVIDENCE.md) read-only and one approved action with post-action verification; one exact-candidate [Desktop Ask](DESKTOP_ASK_EVIDENCE.md) also passed through the installed Host | `PARTIAL` — one OpenAI `gpt-5.6-terra` read-only answer from a foreground Notepad fixture passed through the installed product; no second application or product action workflow is claimed | Ship the bounded browser-to-Word product workflow with cooperative human-input yielding |
| Planner / Executor | `YES` | `PARTIAL` — product-facing `ask` and metadata-oriented `plan run` compose one provider plan, 1-4 observations including bounded semantic `document_text` through the sole Runner boundary, and one tool-free final response; side effects remain unavailable | `YES` | `YES` — [OpenAI and Claude passed](E3_EVIDENCE.md) the earlier bounded scope, and one exact-candidate OpenAI `gpt-5.6-terra` [document-aware plan](DESKTOP_ASK_EVIDENCE.md) passed | `PARTIAL` — one Windows 11 foreground `document_text -> final_response` plan passed from a clean wheel; no desktop action is included | `PARTIAL` — one synthetic Notepad document answer recovered the fixture-only codename, arithmetic, and decision without manual correction | Retain the model-driven public-browser-to-disposable-Word workflow; side-effect expansion remains a separate review |
...
1. **Desktop Ask product vertical:** the installed-first-run configuration,
   ...
   flow now have one exact-candidate Windows/OpenAI/Notepad
   [result](DESKTOP_ASK_EVIDENCE.md). The next product gate is a model-driven
   public-browser-to-disposable-Word workflow; this one result does not broaden
