...
| Campaign | `YES` | `PARTIAL` — the synthetic seam and host-status projection remain bounded; fixed BOSS commands accumulate stable public keys, start the coordinator-selected batch, verify and digest-commit one exact claimed identity through one Runner/project-MCP snapshot, write handoff, then transfer ownership to a fresh zero-port run and claim the exact next item; a separate strict semantic-result contract is offline-only | `YES` — discovery refusal/reconstruction, exact-plan start/claim, identity-drift refusal, single-call commit/handoff, fresh/stale owner transfer/resume, trace privacy, zero-selector CLI behavior, and the [semantic schema/ladder](BOSS_SEMANTIC_EXTRACTION_CONTRACT.md) are tested | `NO` | `PARTIAL` — the [synthetic path](SYNTHETIC_CAMPAIGN_EVIDENCE.md), [two-pass discovery](BOSS_CAMPAIGN_MULTIPAGE_EVIDENCE.md), historical [three-item diagnostic](BOSS_ITEM_RESTART_DIAGNOSTIC_EVIDENCE.md), and clean [three-item restart sequence](BOSS_ITEM_RESTART_CLEAN_EVIDENCE.md) passed on-device | `PARTIAL` — the clean gate retained twelve identities and three consecutive fresh-run commits with zero correction, provider calls, tokens, retryable items, or uncertain items; semantic extraction, provider rotation, and the 100-item gate remain open | Connect the offline semantic contract to a reviewed multi-call item runtime; no general worker |
| Observation | `YES` | `PARTIAL` — UIA, full primary-display screenshot, bounded region OCR, bounded region image capture, bounded UIA document text, bounded Agent image handling, and a pure BOSS per-item ladder reducer exist; delta observations do not | `YES` — OCR, capture, document-text limits, schemas, redaction, timeout, result projection, and strict ladder escalation/handoff are tested | `NO` | `PARTIAL` — [BOSS OCR evidence](BOSS_OCR_EVIDENCE.md) recovered a missing static tab and matched one job card to UIA; bounded [document text](DOCUMENT_TEXT_EVIDENCE.md) and a synthetic [region capture](CAPTURE_REGION_EVIDENCE.md) passed on-device | `PARTIAL` — the clean restart gate verified three public identities, but did not extract job semantics or exercise per-item OCR/document-text fallback | Connect the offline-verified ladder through Runner and retain one semantic item |
| Operator UI | `YES` | `PARTIAL` — console approval, an opt-in four-choice focus-taking Decision Card with expandable digest evidence through the existing ApprovalPort, same-run re-observe, durable non-resumable defer, passive progress, and opt-in durable ordinary-run presence exist; broader presence wiring, public status, notifications, and general process resume do not | `YES` — [re-observe/defer evidence](DECISION_CARD_RECOVERY_EVIDENCE.md) covers zero-dispatch decisions, stale-turn abandonment, fresh-observation gating, `PAUSED` persistence/recovery projection, and the sole Runner dispatch boundary; progress/presence tests cover redaction and teardown | `NO` | `PARTIAL` — native [three-option Decision Card focus/timeout evidence](DECISION_CARD_WINDOW_EVIDENCE.md), progress evidence, and native presence evidence predate the four-choice extension | `NO` | Retain a human-operated four-choice cross-application UX result without widening action authority |
...
3. **Runtime connection:** the fixed campaign seam now reuses the Agent
   ...
   correction. The strict semantic schema and observation ladder are now
   offline verified; next connect them through Runner and retain one semantic
   item before the 100-item gate.
