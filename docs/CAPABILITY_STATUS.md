...
| Operator UI | `YES` | `PARTIAL` — console approval, a focus-taking four-choice Decision Card, passive presence/progress, CLI-first Task Center and receipts, Pre-run Review, cooperative control, Approval Inbox/local notifications, and bounded Windows [accessibility](OPERATOR_ACCESSIBILITY.md) and [English/Simplified-Chinese localization](OPERATOR_LOCALIZATION.md) contracts exist. Safe keyboard traversal, native UIA Text/Edit/Button semantics, bounded status announcements, system High Contrast/reduced motion, 200%/400% text reflow, and locale-neutral authority are implemented and offline verified. Other CLI localization, general process resume, terminal/mobile notifications, native Inbox UI, multi-monitor composition, personalization, and human Narrator/NVDA/live-large-text review remain absent | `YES` — exact approval/re-observe/defer, generated defaults, progress/presence isolation, Task Center receipts, cooperative control, Approval Inbox boundaries, accessibility fallback, contrast, focus/order/action, announcements, UIA names, large-text layout, strict locale resolution, two-locale copy, unknown-text preservation, and stable IDs are covered by deterministic tests | `NO` | `YES` — native [Decision Card](DECISION_CARD_WINDOW_EVIDENCE.md), [progress](PROGRESS_LIFECYCLE_EVIDENCE.md), [presence](PRESENCE_WINDOW_EVIDENCE.md), bounded two-locale UIA/no-activation smoke, and one human-approved fixed [Chrome-to-Word workflow](PUBLIC_WEB_WORD_PRODUCT_EVIDENCE.md) exist; the accessibility/localization smoke is not retained screen-reader or application evidence | `PARTIAL` — Decision Cards authorized the exact fixed cross-application result; accessibility/localization presentation and passive/read-only/attention surfaces do not widen authority or create application evidence | Continue multi-display and personalization as separately scoped feature slices; retain human Narrator/NVDA and live 200%/400% review for the exact feature-freeze candidate |
