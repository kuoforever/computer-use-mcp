...
| Campaign | `YES` | `PARTIAL` — control state exists; one fixed synthetic item can execute `list_windows` through Runner and persist `OBSERVED` plus a bounded non-sensitive `EXTRACTED` count | `YES` | `NO` | `NO` | `NO` | Verify and commit that same fixed synthetic result, then force restart and resume |
...
~~~text
python -m pytest -q              841 passed, 3 skipped
...
3. **Runtime connection:** the fixed campaign seam now reuses the Agent
   authority boundary through correlated `OBSERVED`, then extracts only a
   bounded non-sensitive window count and persists `EXTRACTED`. Verify and
   commit that same result before forced-restart resume, a general worker, or
   more preflights.
