...
| Agent Host | `YES` | `PARTIAL` — installed `config init` / `config doctor`, dual-provider read-only loop, private signed/redacted Claude reasoning-block continuation, locally approved actions, provider request timeout, and fixed `workflow public-web-word` composition. Current `approved_actions` still interrupts for every side effect; Host-owned risk tiers are not implemented | `YES` — doctor fail-fast setup UX, exact thirteen-tool installed-MCP discovery, model-authored brief/save/reopen happy path, stable reviewed Provider contract plus observation-only Runner reacquisition, and wheel resource contract are functional gates | `YES` — [OpenAI and Claude passed](E3_EVIDENCE.md) both bounded fake-MCP E3 cases with reviewed model IDs; Sonnet 5 compatibility is revalidated on the exact repair commit | `YES` — [both reviewed providers passed](E4_EVIDENCE.md) read-only and one approved action with post-action verification; same-wheel current-candidate [Desktop Ask and public-web-to-Word](CURRENT_CANDIDATE_PRODUCT_INTEGRATION_EVIDENCE.md) runs also passed through the installed Host | `PARTIAL` — one OpenAI `gpt-5.6-terra` Notepad answer and one fixed OpenAI/Chrome/Word workflow passed through the installed product; no broader application claim | Implement bounded Host-owned risk tiers so only high-risk/ambiguous effects prompt; keep forbidden effects denied and E4/release separate |
