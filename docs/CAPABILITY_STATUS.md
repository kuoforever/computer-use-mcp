...
| Campaign | `YES` | `PARTIAL` — three fixed CLI commands cover one synthetic claim and durable resume; an internal bounded host-status projection adds deduplicated terminal/attention decisions without a public status tool | `YES` | `NO` | `YES` — [the exact on-device three-command synthetic path passed](SYNTHETIC_CAMPAIGN_EVIDENCE.md) | `NO` | Run the bounded multi-item read-only BOSS campaign with restart evidence; no general worker |
