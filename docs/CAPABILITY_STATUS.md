...
| Operator UI | `YES` | `PARTIAL` — console approval, an opt-in three-choice focus-taking Decision Card with expandable digest evidence through the existing ApprovalPort, passive progress, and opt-in durable ordinary-run presence exist; persisted re-observe/defer semantics, broader presence wiring, public status, and notifications do not | `YES` — Decision Card tests cover fixed rendering and evidence, authority-yield ordering, exact request plus six Host digests, handoff/close/timeout/error/malformed/expiry/drift denial, and the sole Runner dispatch boundary; progress/presence tests cover redaction, native behavior, and teardown | `NO` | `PARTIAL` — model [offline evidence](DECISION_CARD_MODEL_EVIDENCE.md), native [three-option Decision Card focus/timeout evidence](DECISION_CARD_WINDOW_EVIDENCE.md), progress evidence, and native presence evidence are retained | `NO` | Add Host-owned re-observe/defer semantics without widening approval or dispatch authority, then retain a human-operated cross-application UX result |
