...
| Operator experience | Implemented passive progress, opt-in durable ordinary-run presence, and an opt-in focus-taking three-choice Win32 Decision Card with expandable digest evidence through the existing ApprovalPort; persisted re-observe/defer semantics, broader runtime wiring, and host-owned mobile notification remain planned |
