...
| Operator experience | Implemented passive progress, opt-in durable ordinary-run presence, and an opt-in focus-taking four-choice Win32 Decision Card with expandable digest evidence through the existing ApprovalPort; same-run re-observe and durable non-resumable defer are offline verified, while broader runtime wiring and host-owned mobile notification remain planned |
