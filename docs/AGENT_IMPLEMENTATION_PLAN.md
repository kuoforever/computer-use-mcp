...
| Workflow and recovery | Partial | Observe/approve/act/reobserve/answer executes with phase checkpoints; final answer and repeated actions are blocked until verification. Opt-in continuation v1 supports locked one-step recovery for completed-provider-to-observation, completed-observation-to-provider, and completed-side-effect-to-mandatory-`ui_snapshot`. The last path stops after observation and cannot continue the old provider exchange. Unknown outcomes, uncertain dispatches, pending side effects, drift, and broader multi-step recovery remain fail-closed. |
...
| Evaluation and CI | Partial | Windows/Python 3.11-3.13 CI runs Ruff, full offline tests, thirteen-case workflow E1/E2 JSON reports, canonical manifest verification, wheel build, and clean-install CLI smoke. A separate frozen 14-case runtime crash-recovery matrix verifies exact provider/MCP call counts and zero action replay. Provider E3 remains explicit; isolated E4/E5 evidence remains. |
