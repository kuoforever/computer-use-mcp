...
| Evaluation and CI | Partial | Windows/Python 3.11-3.13 CI runs Ruff, full offline tests, a separately reported OpenAI stateless-replay E2 gate, thirteen-case workflow E1/E2 JSON reports, canonical manifest verification, wheel build, and clean-install CLI smoke. A local fail-closed preflight repeats those gates with a minimal reviewed child environment, no pip index/input/config discovery or dependencies downloaded during build/install, reconciles public version sources, rechecks clean source and `HEAD` after the gates, and emits sanitized hashes/counts plus UTC and non-path runtime identity. Preflight report v4 binds the replay gate to its nine-case canonical fixture and manifest SHA-256 values. A frozen 15-case runtime crash-recovery matrix verifies exact provider/MCP call counts, local-only final-response terminalization, and zero action replay; the replay matrix verifies exact wire order, fail-closed transcript rejection, remote-chain preservation, and zero historical MCP dispatch. Provider E3 remains explicit; isolated E4/E5 evidence remains. |
