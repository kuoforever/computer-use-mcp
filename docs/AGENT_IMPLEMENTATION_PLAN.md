...
| Workflow and recovery | Partial | Observe/approve/act/reobserve/answer executes with phase checkpoints; final answer and repeated actions are blocked until verification. Explicit cancel and crash-safe initial resume are implemented. Strict private continuation v1 storage, opt-in runtime provider/MCP write-ahead boundaries, and a conservative crash classifier exist, but runtime recovery consumes none of the new decisions; checkpoints after a provider/tool call remain non-resumable, and unknown outcomes never replay. |
