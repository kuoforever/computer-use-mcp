...
| Task planning | Contract, private persistence, dual-provider Planner, pure preflight/session, first observation runtime, and local completed-observation reconciliation implemented; CLI/provider-final/action execution not connected | The non-executing layers remain strict and non-authoritative. `executor_runtime.py` requires continuation WAL and owns one RunLock, recorder, grounding state, MCP generation, and bounded session across internal observation steps. It CAS-marks a step `in_progress` before the shared Runner boundary writes dispatch intent, then commits `completed`/`failed` from exact results. Unknown outcome retains `in_progress` plus continuation and closes without replay. `executor_reconciliation.py` can repair only a missed terminal CAS when a revalidated completed WAL exactly proves the same known observation result; it retains WAL and performs no external call. Dispatch intent, uncertainty, side effects, drift, broader resume, final-response orchestration, and CLI integration remain separate reviews. |
