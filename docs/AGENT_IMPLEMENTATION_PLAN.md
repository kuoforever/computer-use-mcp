...
| Evaluation and CI | Partial | Windows/Python 3.11-3.13 CI runs Ruff, full offline tests, thirteen-case workflow E1/E2 JSON reports, canonical manifest verification, wheel build, and clean-install CLI smoke. A local fail-closed preflight repeats those gates with a minimal reviewed child environment, no pip index/input/config discovery or dependencies downloaded during build/install, reconciles public version sources, rechecks clean source and `HEAD` after the gates, and emits sanitized hashes/counts plus UTC and non-path runtime identity. A frozen 14-case runtime crash-recovery matrix verifies exact provider/MCP call counts and zero action replay; a separate frozen nine-case stateless-replay matrix verifies exact wire order, fail-closed transcript rejection, remote-chain preservation, and zero historical MCP dispatch. Provider E3 remains explicit; isolated E4/E5 evidence remains. |
