...
| Workflow and recovery | Partial | Observe/approve/act/reobserve/answer executes with phase checkpoints; final answer and repeated actions are blocked until verification. Explicit cancel and crash-safe initial resume are implemented. A strict private continuation v1 storage foundation exists but is not consumed by runtime recovery; checkpoints after a provider/tool call remain non-resumable, and unknown outcomes never replay. |
