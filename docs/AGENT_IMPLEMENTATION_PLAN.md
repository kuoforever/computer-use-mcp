...
| Task planning | Contract, private persistence, one-shot Planner port, isolated OpenAI/Claude adapters, and pure Executor preflight implemented; runtime not connected | `planning.py` provides a strict 64 KiB/16-step JSON compiler, immutable task/registry-digest-bound plans, reviewed tool/argument validation, sensitive-argument exclusion, host-derived approval metadata, and ordered pure transitions. `plan_store.py` adds strict 128 KiB private snapshots with registry/plan/envelope digest verification and atomic sequence/plan-digest compare-and-swap transitions under the existing application RunLock. `planner.py` defines a bounded provider-neutral request with exact host-scoped non-sensitive schemas and a one-shot no-retry candidate port whose output returns to the same compiler. `planner_wire.py` shares the bounded structured-output envelope conversion so provider grammars cannot drift from host validation. The OpenAI and Claude adapters each implement one stateless, tool-free Structured Outputs request with canonical byte/token preflight and fixed refusal/shape failures. `executor.py` revalidates an exact snapshot against current run/task/registry state and reconstructs only a fresh `requested` call for the first pending tool step. It has no ports, does not mutate plan or budgets, and performs no policy, approval, persistence, MCP, or execution. Runtime consumption remains a separate milestone. |
