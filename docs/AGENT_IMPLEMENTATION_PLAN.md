...
| --- | --- | --- | --- |
| 0. Contract and threat model | None | Canonical types, ports, tool-registry specification, trust boundaries, configuration model, and acceptance matrix | Both providers can be described through the same `ToolCall`/`ToolResult` contract; all thirteen tools have reviewed schemas. |
