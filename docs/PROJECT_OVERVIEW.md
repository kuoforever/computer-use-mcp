...
| Presence and progress UI | Partial | Passive progress is implemented and follows durable ordinary `run`/`resume`, bounded `plan run`, and explicit read-only recovery phases; an opt-in bounded primary-display halo is [desktop verified](PRESENCE_WINDOW_EVIDENCE.md) and follows only ordinary `run`/`resume` phases through a fail-silent Host coordinator. Campaign wiring and integrated plan/recovery desktop evidence remain planned | [Operator experience](OPERATOR_EXPERIENCE.md), [Progress viewer](PROGRESS_VIEWER.md) |
