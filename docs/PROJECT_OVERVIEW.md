...
| Presence and progress UI | Partial | Passive progress is implemented and follows durable ordinary `run`/`resume`, bounded `plan run`, explicit read-only recovery phases, and validated state during fixed MCP-backed campaign execution; zero-port campaign control remains window-free. An opt-in bounded primary-display halo is [desktop verified](PRESENCE_WINDOW_EVIDENCE.md) and follows only ordinary `run`/`resume` phases through a fail-silent Host coordinator. Integrated plan/recovery/campaign desktop evidence remains planned | [Operator experience](OPERATOR_EXPERIENCE.md), [Progress viewer](PROGRESS_VIEWER.md) |
