...
| Decision Cards | Partial / opt-in Windows | Pure cards compile 2-3 bounded options; an opt-in focus-taking two-choice Win32 adapter yields authority and returns exact-effect approval/denial through the existing ApprovalPort. Richer business choices and cross-application evidence remain planned | [Operator experience](OPERATOR_EXPERIENCE.md), [Approved actions](APPROVALS.md) |
