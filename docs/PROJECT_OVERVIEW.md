...
| Presence and progress UI | Partial | Passive progress is implemented and follows durable ordinary `run`/`resume` plus bounded `plan run` phases; an opt-in bounded primary-display halo is [desktop verified](PRESENCE_WINDOW_EVIDENCE.md) and follows only ordinary `run`/`resume` phases through a fail-silent Host coordinator. Campaign/recovery wiring and integrated plan-run desktop evidence remain planned | [Operator experience](OPERATOR_EXPERIENCE.md), [Progress viewer](PROGRESS_VIEWER.md) |
