...
| BOSS identity discovery | Implemented/internal fixed runtime | Two fixed CLI commands create the reviewed manifest and dispatch one foreground `ui_snapshot` through Runner/project MCP; bounded complete link values produce idempotent public job keys with query data discarded. Fake-MCP verified only; no navigation, worker, or application evidence | [Long-running tasks](LONG_RUNNING_TASKS.md) |
