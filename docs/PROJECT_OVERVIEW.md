...
| Presence and progress UI | Partial | Passive progress is implemented and follows durable ordinary `run`/`resume`, bounded `ask` / `plan run`, explicit read-only recovery phases, and validated state during fixed MCP-backed campaign execution; zero-port campaign control remains window-free. System High Contrast, reduced motion, bounded UIA status names, 200%/400% text reflow, and bounded English/Simplified-Chinese presentation are implemented and offline verified; human assistive-technology and live large-text review remain open. The fixed synthetic campaign lifecycle is [desktop verified](CAMPAIGN_PROGRESS_LIFECYCLE_EVIDENCE.md); one persisted read-only observation has separate [recovery progress evidence](RECOVERY_PROGRESS_LIFECYCLE_EVIDENCE.md); and one fixed provider-free plan has separate [plan progress](PLAN_PROGRESS_LIFECYCLE_EVIDENCE.md) and [presence](PLAN_PRESENCE_LIFECYCLE_EVIDENCE.md) lifecycle evidence. The configurable bounded primary-display halo is also [desktop verified](PRESENCE_WINDOW_EVIDENCE.md). Integrated BOSS-campaign progress/presence evidence, recovery presence desktop evidence, and multi-display composition remain planned | [Operator experience](OPERATOR_EXPERIENCE.md), [Operator accessibility](OPERATOR_ACCESSIBILITY.md), [Operator localization](OPERATOR_LOCALIZATION.md), [Progress viewer](PROGRESS_VIEWER.md) |
