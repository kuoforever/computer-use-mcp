...
`plan run`, and explicit `recover --execute-read-only` can show the
primary-display computer-use halo with an explicit local opt-in:
...
integration does not yet cover campaign runtimes and remains
