...
fails closed.
For the exact Qwen `cn-beijing` + `qwen3.7-plus` one-shot Planner only, the
adapter may remove one exact lowercase Markdown JSON fence with LF-only
boundaries after checking the original UTF-8 response against the existing byte limit. The
unwrapped text still enters the unchanged Host compiler. Extra prose,
whitespace, case changes, nested fences, every sibling route/model, ordinary
continuation, and final response retain the prior fail-closed behavior.
