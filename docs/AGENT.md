...
| Host completion projection is evidence-only | A future Codex/Claude host may read bounded durable campaign status and finish only on validated terminal state; waiting states request attention, uncertainty forbids replay, and MCP log notifications are never terminal evidence | the fixed synthetic CLI path now has [retained on-device evidence](SYNTHETIC_CAMPAIGN_EVIDENCE.md); the status tool, notification bridge, and mobile adapter remain unimplemented |
