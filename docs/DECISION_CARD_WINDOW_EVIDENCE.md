...
> **Status: responsive four-choice native approval smoke passed 2026-07-26.**
...
- Original approval-boundary commit:
  `ee1afab49d0050650a01bed9dffe9878ce8cab58`
- Command: `.venv\Scripts\python.exe scripts\smoke_decision_card.py`
- The default command remains fully automated and uses fake provider/desktop
  ports. `--human` is an explicit optional UX probe that waits for the operator
  to approve the same synthetic exact effect; it is not required for ordinary
  local regression.
...
- Window scope: a compact bottom-right normal overlapped window with standard
  drag, resize, minimize, and maximize styles; no topmost style; responsive
  one/two-column option layout; read-only vertically scrollable decision and
  evidence panes; and configurable placement in any work-area corner.
- Selection scope: UI Automation verified the four custom choices and evidence
  affordance, the probe resized the first card and programmatically selected
  the exact-effect button, and the second card's native five-second timeout
  expired.
...
~~~text
RESULT: PASS (card foreground 0xaf04f2; authority yielded; normal movable/resizable non-topmost window and scrollable content verified; four bounded options and evidence affordance rendered; approved choice used ordinary dispatch and verification; five-second timeout denied with zero side-effect dispatch; prior foreground restored)
~~~
...
observed exact-effect approval, re-observe, defer, denial, and the collapsed
`Show bounded evidence` control on the normal native window. The smoke also
proved the resize styles, actual size change, absence of topmost, and vertical
scroll capability. The second run closed
...
- the native card offers exact-effect approval, re-observe, defer, and denial;
  it has no recommendation and keeps the console approval path as the default;
...
real process-to-process MCP behavior, a human-selected handoff,
resumable defer/re-observe semantics, or application acceptance. Windows-only
four-choice presentation, one monitor work area, and one exact action remain the
