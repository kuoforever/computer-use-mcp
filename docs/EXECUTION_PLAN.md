...
implemented without an approval or dispatch port. Next connect a focus-taking
local card through the existing `ApprovalPort`. Do not display
...
| Operator experience | The console supports one-action yes/deny; passive progress and the primary-display presence lifecycle are retained; fake-only Decision Card compilation and selection now provide bounded fixed trade-offs, estimate provenance, safe exits, advisory recommendation, expiry, and complete digest-drift invalidation without any approval or dispatch port. No interactive card exists | Connect one focus-taking local Decision Card through the existing ApprovalPort without a second dispatch path, default closed on timeout/window close/malformed response, and separately retain evidence before broader presence wiring |
