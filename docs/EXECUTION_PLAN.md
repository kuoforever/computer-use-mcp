...
| Token-aware context | Event-count reduction, exact request-byte gates, cumulative provider-reported input-token cutoff, conservative provider/model pre-request enforcement, correlated OpenAI recovery token-state restoration, Claude-only oldest-complete-group packing, canonical OpenAI request-contract digest binding, and an explicit fail-closed stateless-replay readiness contract | Tokenizer-specific calibration, safe semantic compression, exact original-request/provider-output persistence, and a reviewed replay compiler before any OpenAI remote-chain compaction |
