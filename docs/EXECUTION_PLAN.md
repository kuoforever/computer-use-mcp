...
| Planner-Executor | Strict TaskPlan compilation, private CAS persistence, isolated dual-provider Planner adapters, pure fresh-call preflight, and a four-step lock-scoped session contract are implemented. `RuntimeExecutorSession` now provides the first real observation-only execution path: continuation/WAL is mandatory; one RunLock, recorder, grounding state, MCP generation, and session contract live across steps; each plan step becomes `in_progress` before WAL; the runtime has no direct MCP dispatch and uses the sole Runner boundary; success/known failure commit exact terminal step transitions; unknown outcome retains `in_progress`, preserves continuation, closes, and is never replayed. Side effects, provider/final-response execution, CLI wiring, and implicit resume remain unavailable | Add provider/final-response orchestration and explicit resumable session design before CLI wiring; keep the four-step cap, require the same shared host boundary for any future side effect, and add action execution only with unchanged approval/grounding/verification semantics and isolated evidence |
