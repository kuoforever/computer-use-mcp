...
terminal state, or cleanup. Next retain its on-device lifecycle result, then
deliver fake-only Decision Card models from
...
| Operator experience | The console supports one-action yes/deny; passive progress with run/campaign grouping is implemented; the primary-display presence model/controller/Win32 halo is offline and [desktop verified](PRESENCE_WINDOW_EVIDENCE.md); and a default-off fail-silent coordinator now follows durable ordinary `run`/`resume` phases with authority-boundary latching. Broader runtime wiring and Decision Cards remain planned | Retain one on-device ordinary Host lifecycle transition and teardown result; Decision Cards then present bounded alternatives and provenance-backed trade-offs, invalidate on drift, and connect through the existing ApprovalPort without a second dispatch path |
