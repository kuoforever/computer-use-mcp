...
| Planner-Executor | A strict, bounded, immutable TaskPlan contract and non-executable JSON candidate compiler bind task/registry digests, derive tool effect/approval metadata, forbid sensitive arguments, and enforce ordered local transitions. Private plan snapshots are strict, bounded, digest-bound, atomically replaced only while the application RunLock is held, and require exact sequence/plan-digest compare-and-swap validation. A one-shot provider-neutral PlannerPort accepts only a bounded task plus host-scoped exact non-sensitive tool schemas and returns untrusted JSON to the same compiler. Isolated OpenAI and Claude Structured Outputs adapters each add one stateless, tool-free, no-retry request with complete byte/token preflight and fixed refusal/shape failures. A pure Executor preflight rejects run/task/registry/snapshot drift, non-pending or final steps, and reused identities before reconstructing one fresh `requested` call. The ordinary Runner now has one shared requested-call boundary containing policy, grounding, budgets, approval, write-ahead, MCP, result validation, and verification; existing provider workflows use it and no plan is connected | Add a lock-scoped Executor session that rereads/CAS-transitions plans and sends each fresh requested call only through the shared Runner boundary, while preserving recorder/WAL lifetime across steps and keeping plan state non-authoritative |
