...
| Token-aware context | Event-count reduction, exact request-byte gates, cumulative provider-reported input-token cutoff, conservative provider/model pre-request enforcement, correlated OpenAI recovery token-state restoration, and Claude-only oldest-complete-group packing without splitting mandatory groups | Tokenizer-specific calibration, safe semantic compression, and an explicit stateless-replay design before any OpenAI remote-chain compaction |
