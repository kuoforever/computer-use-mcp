...
| Token-aware context | Event-count reduction, exact request-byte gates, cumulative provider-reported input-token cutoff, conservative provider/model pre-request enforcement, correlated OpenAI recovery token-state restoration, Claude-only oldest-complete-group packing, and an explicit fail-closed OpenAI stateless-replay readiness contract | Tokenizer-specific calibration, safe semantic compression, and implementation of every reviewed replay blocker before any OpenAI remote-chain compaction |
