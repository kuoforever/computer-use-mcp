...
DPI geometry, reduced motion, and release/E-stop teardown. Next wire it through
one Host-owned lifecycle coordinator, then deliver fake-only Decision Card
models from [Operator experience](OPERATOR_EXPERIENCE.md). Do not display
...
| Operator experience | The console supports one-action yes/deny; passive progress with run/campaign grouping is implemented, and the standalone primary-display presence model/controller/Win32 halo is offline verified and [desktop verified](PRESENCE_WINDOW_EVIDENCE.md) for non-activation, transparent hit testing, capture affinity, DPI geometry, accessibility modes, and release/E-stop teardown; automatic lifecycle wiring and Decision Cards remain planned | Presence follows one real Host run through phase transitions and teardown without a second authority path; Decision Cards then present bounded alternatives and provenance-backed trade-offs, invalidate on drift, and connect through the existing ApprovalPort without a second dispatch path |
