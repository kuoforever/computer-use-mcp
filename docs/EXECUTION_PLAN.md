...
| Planner-Executor | A strict, bounded, immutable TaskPlan contract and non-executable JSON candidate compiler bind task/registry digests, derive tool effect/approval metadata, forbid sensitive arguments, and enforce ordered local transitions. Private plan snapshots are strict, bounded, digest-bound, atomically replaced only while the application RunLock is held, and require exact sequence/plan-digest compare-and-swap validation | Add a separately reviewed Planner port, then a bounded Executor; a persisted plan remains data and must never grant action authority |
