...
| --- | --- | --- |
| Formal Demo v1 | [Formal Demo v1](FORMAL_DEMO_V1.md) owns the selected independent Agent Console and GitHub Issues -> PDF -> disposable Excel -> disposable Word -> Outlook Desktop test-account email-draft story. Internal v1 intent/scenario/profile/Scope, typed local disclosure/exact-`COMPILE` permit, async one-attempt coordinator, exact OpenAI Responses live-capable intent port plus internal Provider Scope result, fixed Host-owned no-key compiler, five inert selected role pins, and an independent Offline Scope Review Console/launcher are implemented. The Provider path is offline verified but not Console-connected or credentialed-live verified. The Console consumes one local permit, displays only the local complete reviewed Scope with zero provider calls/retries, and stops with native `Start` disabled; executable application adapters, durable composition, and complete evidence remain absent | Product-demo delivery program. It may reuse only separately verified mechanisms and cannot inherit evidence from the fixed public-web-word workflow |
