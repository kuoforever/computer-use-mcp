...
| Planner-Executor | Strict TaskPlan compilation, private CAS persistence, isolated dual-provider Planner adapters, pure fresh-call preflight, and a four-step lock-scoped session contract are implemented. `RuntimeExecutorSession` provides the first real observation-only execution path through the sole Runner boundary. A separate local reconciliation path can repair only a known-completed WAL observation whose exact terminal plan CAS was missed; it revalidates run/task/registry/snapshot/call/result bindings, retains WAL, performs zero external calls, and rejects dispatch intent or uncertainty without mutation. Side effects, provider/final-response execution, CLI wiring, broader resume, and implicit resume remain unavailable | Add provider/final-response orchestration and broader explicit resumable session state before CLI wiring; keep the four-step cap, require the same shared host boundary for any future side effect, and add action execution only with unchanged approval/grounding/verification semantics and isolated evidence |
