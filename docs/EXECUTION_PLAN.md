...
| Broader resume | Strict private continuation storage, opt-in runtime provider/MCP write-ahead boundaries, conservative reconstruction classification, frozen 14-case E2 matrix, provider export/restore, strict pure planning, and an internal commit-before-call one-step executor exist for the two completed read-only boundaries; no runner/CLI path invokes it | Add atomic durable intent/completion integration under the run lock, then expose the reviewed entry point and freeze its end-to-end no-replay assertions |
