...
| Planner-Executor | A strict, bounded, immutable TaskPlan contract and non-executable JSON candidate compiler bind task/registry digests, derive tool effect/approval metadata, forbid sensitive arguments, and enforce ordered local transitions. Private plan snapshots are strict, bounded, digest-bound, atomically replaced only while the application RunLock is held, and require exact sequence/plan-digest compare-and-swap validation. A one-shot provider-neutral PlannerPort accepts only a bounded task plus host-scoped exact non-sensitive tool schemas and returns untrusted JSON to the same compiler. Isolated OpenAI and Claude Structured Outputs adapters each add one stateless, tool-free, no-retry request with complete byte/token preflight and fixed refusal/shape failures. Pure Executor preflight reconstructs only fresh `requested` calls. A non-executing `BoundedExecutorSession` now holds one live PlanStore lock, prepares at most four observation calls with host-generated identities, permits one outstanding call, requires monotonic full-ledger state plus exact result/plan-transition evidence, and closes on unknown outcome. The ordinary Runner has one shared requested-call boundary containing every execution authority; no plan call reaches it yet | Add the runtime session that owns recorder/continuation lifetime, marks the plan step in progress before WAL, sends each fresh requested call only through the shared Runner boundary, commits exact success/failure transitions afterward, and leaves uncertain work permanently non-replayable |
