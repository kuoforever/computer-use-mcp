...
| Token-aware context | Event-count reduction, exact request-byte gates, cumulative provider-reported input-token cutoff, and conservative provider/model pre-request window enforcement without splitting mandatory atomic groups | Tokenizer-specific calibration and safe semantic compression |
