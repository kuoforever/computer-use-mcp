...
ordinary `run`/`resume` lifecycle and bounded observation-only `plan run`
Executor path, and latches off on MCP E-stop, human activity, terminal state,
or cleanup. The
