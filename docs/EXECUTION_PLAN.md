...
| Planner-Executor | Strict TaskPlan compilation/persistence, Planner adapters, fresh-call preflight/session, observation runtime/reconciliation, and bounded final-response compilation/adapters are implemented. Final adapters now return correlated response identity/text/usage, and a separate RunLock-scoped atomic WAL records only prepared/dispatch-intent/completed final-response state without entering ordinary provider recovery. It remains disconnected from provider execution orchestration, host budget consumption, plan final-step CAS, terminal trace, recovery, and CLI | Add a separately reviewed ordering boundary across plan in-progress, dedicated WAL, provider call, budget, terminal CAS/trace, and cleanup; keep dispatch intent non-replayable, output untrusted, the four-step cap, and unchanged action boundaries |
