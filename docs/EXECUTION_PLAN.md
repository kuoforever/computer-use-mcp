...
| --- | --- | --- |
| OpenAI and Claude E3 | [Both providers passed](E3_EVIDENCE.md) the ordinary and exact bounded `plan run` fake-MCP cases with reviewed model IDs. The record is model-scoped and preserves the historical Sonnet 5 failure; strict signed/redacted reasoning-block continuation is now implemented and offline verified | Retain an exact-commit Sonnet 5 rerun for the compatibility repair, then proceed to isolated E4 without inferring all-model or desktop compatibility |
