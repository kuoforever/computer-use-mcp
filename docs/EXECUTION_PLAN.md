...
| Broader resume | Controlled one-step recovery atomically persists intent/completion under the run lock; completed side effects can issue one synthetic `ui_snapshot`, then stop without provider continuation. The frozen 14-case runtime E2 matrix proves exact new-call counts and zero action replay | Keep uncertain dispatches and pending side effects permanently non-executable; separately design any broader multi-step continuation |
