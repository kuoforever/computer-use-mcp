...
moving from Active to Attention after pause. Next deliver the click-through
...
| Operator experience | The console supports one-action yes/deny; the progress viewer's pure reducer, passive non-activating window, atomic live polling, fixed independent-run grouping, and lock-free validated campaign grouping are implemented and offline verified, with retained live-desktop evidence for [non-activation, real checkpoint polling, two-run regrouping, and campaign Active-to-Attention regrouping](PROGRESS_POLLER_EVIDENCE.md); the presence indicator and Decision Cards are planned only | Computer-use presence passes focus/capture/DPI/accessibility tests; Decision Cards present bounded alternatives and provenance-backed trade-offs, invalidate on drift, and connect through the existing ApprovalPort without a second dispatch path |
