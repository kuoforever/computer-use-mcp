...
| --- | --- | --- |
| OpenAI and Claude E3 | [OpenAI passed](E3_EVIDENCE.md) the ordinary and exact bounded `plan run` fake-MCP cases with a reviewed model ID; Claude is not run | Claude passes both harmless fake-MCP cases with a reviewed model ID, completing the dual-provider record |
