"""Opt-in E3 matrix: live GLM CN API plus a harmless fake MCP child."""
...
pytestmark = pytest.mark.glm_integration
_GLM_INTEGRATION_MODEL = "glm-5.2"


def _fail_if_credential_present(material: str | bytes) -> None:
    """Fail with fixed prose rather than reflecting sensitive material."""

    credential = os.environ.get("ZAI_API_KEY")
    if not credential:
        pytest.fail("GLM credential is unavailable for leak inspection")
    contains_credential = (
    ...
    if contains_credential:
        pytest.fail("GLM credential leaked into captured test material")
...
def _require_opt_in() -> str:
    if os.environ.get("RUN_GLM_INTEGRATION") != "1":
        pytest.skip("set RUN_GLM_INTEGRATION=1 to enable the live GLM test")
    if not os.environ.get("ZAI_API_KEY"):
        pytest.skip("ZAI_API_KEY is required for the live GLM CN test")
    configured_model = os.environ.get("GLM_INTEGRATION_MODEL")
    if configured_model not in {None, _GLM_INTEGRATION_MODEL}:
        pytest.fail("GLM_INTEGRATION_MODEL must be exactly glm-5.2")
    pytest.importorskip("openai", reason="install the agent-openai optional dependency")
    return _GLM_INTEGRATION_MODEL
...
def test_glm_cn_setup_and_doctor_use_the_formal_product_route(tmp_path: Path) -> None:
    """Generate and diagnose the user-facing GLM CN configuration."""

    _require_opt_in()
    local_app_data = tmp_path / "LocalAppData"
    config_path = tmp_path / "glm-cn-setup.toml"
    ...
    assert setup_payload["configuration"]["provider"] == "glm"
    assert setup_payload["configuration"]["model"] == _GLM_INTEGRATION_MODEL
    ...
    assert doctor_payload["provider"] == "glm"
...
def test_live_glm_cn_tool_result_continuation_cycle(
    ...
    mcp = _mcp_config(tmp_path, "glm-cn-e3")
    config = AgentConfig(
        state_dir=local_app_data / "computer-use-agent" / "glm-cn-integration",
        policy_version="glm-cn-e3-v1",
...
def _run_planned_observation(
    ...
    *,
    ...
    local_app_data = tmp_path / "LocalAppData"
    state_dir = local_app_data / "computer-use-agent" / f"glm-plan-{tool_name}"
    fixture = Path(__file__).parent / "fixtures" / "stdio_mcp_server.py"
    child_cwd = tmp_path / f"glm {tool_name} plan child"
    child_cwd.mkdir()
    config_path = tmp_path / f"glm-{tool_name}-plan.toml"
    config_path.write_text(
        f'''\
[agent]
state_dir = {json.dumps(state_dir.as_posix())}
policy_version = "glm-cn-plan-e3-v1"
...
args = [{json.dumps(fixture.as_posix())}, "glm-cn-plan-e3"]
...
def test_live_glm_cn_structured_planner_and_final_cycle(tmp_path: Path) -> None:
...
def test_live_glm_cn_withdraws_image_tools_from_the_model_request(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    local_app_data = tmp_path / "LocalAppData"
    monkeypatch.setenv("LOCALAPPDATA", str(local_app_data))
    mcp = _mcp_config(tmp_path, "glm-cn-text-only-e3")
    config = AgentConfig(
        state_dir=local_app_data / "computer-use-agent" / "glm-cn-text-only",
        policy_version="glm-cn-text-only-e3-v1",
        provider=_provider(model),
        mcp=mcp,
    ...
    advertised_names = {definition["function"]["name"] for definition in definitions}
...
def test_live_glm_cn_timeout_stops_before_mcp_dispatch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    local_app_data = tmp_path / "LocalAppData"
    monkeypatch.setenv("LOCALAPPDATA", str(local_app_data))
    mcp = _mcp_config(tmp_path, "glm-cn-timeout-e3")
    config = AgentConfig(
        state_dir=local_app_data / "computer-use-agent" / "glm-cn-timeout",
        policy_version="glm-cn-timeout-e3-v1",
