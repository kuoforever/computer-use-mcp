"""Opt-in E3 test: live OpenAI Responses API plus a harmless stdio MCP child."""
...
def _require_opt_in() -> str:
    if os.environ.get("RUN_OPENAI_INTEGRATION") != "1":
        pytest.skip("set RUN_OPENAI_INTEGRATION=1 to enable the live OpenAI test")
    if not os.environ.get("OPENAI_API_KEY"):
        pytest.skip("OPENAI_API_KEY is required for the live OpenAI test")
    model = os.environ.get("OPENAI_INTEGRATION_MODEL")
    if not model:
        pytest.skip("OPENAI_INTEGRATION_MODEL is required to pin cost and behavior")
    pytest.importorskip("openai", reason="install the agent-openai optional dependency")
    return model


def test_live_openai_read_tool_result_final_answer_cycle(
    ...
    child_cwd = tmp_path / "openai integration child"
    child_cwd.mkdir()
    config = AgentConfig(
        state_dir=local_app_data / "computer-use-agent" / "openai-integration",
        policy_version="openai-e3-v1",
        provider=ProviderConfig(name="openai", model=model),
        mcp=MCPLaunchConfig(
            executable=Path(sys.executable).resolve(),
            args=(str(fixture), "openai-e3"),
            cwd=child_cwd,
    ...
    runner = AgentRunner(
        config,
        RunnerPorts(
            provider=OpenAIResponsesProvider.from_environment(model),
            desktop=desktop,
