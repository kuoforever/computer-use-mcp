...
> scope-grammar defect was repaired. `GDA-PRODUCT-004` merged through PR #269
> as `0275f25`: one exact-candidate installed, model-driven
> public-browser-to-disposable-Word workflow passed without importing draft PR
> #231 wholesale. `GDA-PRODUCT-005` is now active only for the user's explicit
> default-on UI/UX requirement before its separate exact-candidate release
> gates. `GDA-CORE-029` closed inside
