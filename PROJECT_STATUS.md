...
> The user explicitly continued delivery. `GDA-PRODUCT-014` non-E4
> feature-freeze evidence merged through PR #281 as `837a42f`, and both
> feature-branch copies were removed. `GDA-PRODUCT-015` same-wheel
> current-candidate Notepad and fixed Chrome-to-Word integration evidence
> merged through PR #282 as `3ddc8f8`; both feature-branch copies were removed.
> `GDA-PRODUCT-016` bounded Host-owned risk-tier approval policy merged through
> PR #283 as `7184dee`; both feature-branch copies were removed.
> `GDA-PRODUCT-017` automated non-E4 evidence merged through PR #284 as
> `0f559ec`; both feature-branch copies were removed. Supervised English
> Narrator review on 2026-08-08 found and repaired one focus-triggered
> 500-plus-character details dump, then passed short interactive Tab order and
> on-demand scan-mode reading. The subsequent cross-surface UX walkthrough
> accepted the Decision Card, found and repaired Progress/Presence 200%/400%
> overflow and semantic-control defects, and passed ten current native cases.
> Exact next is human confirmation of the revised passive surfaces;
> visible-notification confirmation and real operator takeover timing follow.
> Those two later gates remain `NOT RUN`, while
...
| `GDA-PRODUCT-017` | Automatic subset merged; human gates in progress | Remaining available non-E4 human/native acceptance | PRODUCT-016 merge `7184dee` and PR #284 supplied the complete automatic baseline. Supervised English Narrator review reproduced and repaired a 500-plus-character focus-triggered details dump. A later cross-surface heuristic/cognitive walkthrough accepted the repaired Decision Card, found real Progress/Presence large-text defects, and repaired them with a wrapping Document plus real presentation-only Button and measured Presence glyph extents. Ten current native safe-denial cases pass with fake-only ports and no approval. [Automatic evidence](docs/PRODUCT017_AUTOMATED_NATIVE_EVIDENCE.md) and [human evidence](docs/PRODUCT017_HUMAN_NATIVE_EVIDENCE.md) keep revised passive-surface human confirmation, visible notification, native operator timing, and other AT/locales open; physical two-monitor is hardware-blocked and E4/release remain deferred. Exact next: confirm revised Progress/Presence at 200%/400% |
...
| Offline baseline | Run the current suite; CI publishes the live total. Last full gate: 2026-08-07, all green — `2038 passed, 8 skipped`, Ruff, mypy over 138 source files, docs consistency, and `git diff --check` |
...
| `GDA-PRODUCT-017` | Automatic subset merged; human gates in progress | Remaining available non-E4 human/native acceptance | PR #284 merged the complete automatic baseline as `0f559ec`. The supervised English Narrator path and Decision Card large-text design passed after bounded repairs. The cross-surface walkthrough then repaired Progress clipping/non-semantic disclosure and Presence glyph-width overflow; ten current native cases pass. Revised Progress/Presence human confirmation, visible notification, and operator timing remain open; other AT/locales remain unclaimed, physical two-monitor is hardware-blocked, and E4/release remain deferred. Exact next is revised passive-surface visual confirmation |
...
`GDA-PRODUCT-017` automated non-E4 evidence merged through PR #284 as
`0f559ec`; its remaining human gates are actively in progress.
