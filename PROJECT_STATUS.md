...
| `GDA-HUD-005` | Step semantics | The surfaces mix different totals: Progress uses the Host tool-call budget while the card shows seven approval actions. The progress view cannot yet name the exact current Demo chapter from durable Host state. | A bounded immutable checklist contract defines six fixed Demo chapters and can render an explicitly supplied compact summary with completed/skipped/not-started counts, exact current chapter, application, and overall state. Approval and tool budgets are absent from this mode. Durable Demo transition wiring is not started. | The UI clearly labels “workflow step” versus “approval n/7”, names the current fixed action without trusting provider prose, and defines how skipped, failed, verification, and terminal steps affect counts. |
