...
| `GDA-HUD-005` | Step semantics | The surfaces mix different totals: Progress uses the Host tool-call budget while the card shows seven approval actions. The progress view cannot yet name the exact current Demo chapter from durable Host state. | A bounded immutable checklist defines six fixed Demo chapters and drives compact/expanded Progress projections. Decision Card can derive one current breadcrumb from that validated checklist while keeping the exact approval action and `n/7` count separate. The pure `project_demo_workflow` mapper now fail-closed maps fixed provider boundaries `0..18` to the six Host-owned chapters, including terminal readiness; targeted tests pass. It is not yet connected to the UI-owning Progress lifecycle or real Decision Card context, so no live/durable transition claim is made. | The UI clearly labels “workflow step” versus “approval n/7”, names the current fixed action without trusting provider prose, and defines how skipped, failed, verification, and terminal steps affect counts. |
...
| `GDA-HUD-011` | Evidence and promotion | The visual changes have no retained screenshot/video matrix or successful end-to-end run. The latest run is a failure and must not replace prior passing evidence. | [A four-image isolated matrix](docs/OPERATOR_HUD_VISUAL_EVIDENCE.md) now retains Decision compact/expanded and Progress default-checklist/operator-collapsed states at 144 DPI (150%). Computer Use inspected the same synthetic surfaces. The fail-closed capture helper uses the project DPI contract, exact white-listed titles, state geometry, a compositor-settle interval, and fixed dated outputs; duplicate visual-review instances are rejected. The complete offline gate passes (`1527 passed, 8 skipped`; Ruff, mypy, docs consistency, and diff checks pass). Presence, 100%/125% DPI, durable Demo workflow wiring, and a complete Chrome-to-Word run remain. | Each issue has proportional offline tests, a dedicated live smoke where visual behavior matters, one complete retained Chrome-to-Word run, documented DPI/keyboard evidence, full validation gate, and explicit statement that the result remains bounded rather than universal GUI evidence. |
