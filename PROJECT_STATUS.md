...
| `GDA-HUD-007` | Cross-surface visual system | Presence, Progress, and Decision Card do not yet feel like states of one product. Approval has no strong amber relationship to the phase halo, and fixed colors/type/spacing are inconsistent. | One fixed token contract now owns operator labels, glyphs, and RGB roles. Presence and workflow Progress consume it directly; generic run/campaign diagnostics use the same vocabulary; Decision Card uses the same amber `Needs input` token plus `approval locked`. Computer Use confirmed matching amber Progress and Decision surfaces at the current DPI. Presence capture remains excluded by design, and full typography/spacing/motion acceptance plus retained evidence remain. | Shared typography, spacing, phase colors, status vocabulary, and motion rules are implemented; approval transition is visually obvious; reduced-motion and high-contrast behavior remain valid. |
...
| `GDA-HUD-009` | Foreground and window composition | The current application must remain foreground while passive HUD surfaces stay visible and non-interactive; the card must restore that application after any exit. Overlap with Chrome/Word and screen-edge placement are not yet composition-tested across DPI. | Progress now defaults to the current foreground monitor's top-right work-area rail and preserves that right edge while its checklist expands or collapses; an explicit operator move disables automatic anchoring. Decision Card remains on the same monitor's bottom-right rail and restores the captured prior foreground on every exit. Pure geometry covers 100%, 125%, and 150% DPI against the bounded Demo application rectangle. Isolated Computer Use at the current DPI confirmed that passive Progress did not replace the Codex foreground, Decision Card alone took focus, and `Esc` removed the card and restored the exact prior Codex window. Chrome/Word and multi-DPI live composition remain. | Live tests prove Chrome/Word remain foreground during passive updates, Decision Card alone takes focus after yield, prior foreground is restored, and no surface obscures the task-critical region. |
| `GDA-HUD-010` | Restart and cleanup | Fresh browser/document state is offline verified, but a failed or cancelled HUD run can leave launched Chrome/Word fixtures open. The next session needs an explicit cleanup/restart contract. | The initial manifest now declares exact-process cleanup. Word is launched as a separate `/x` instance; Chrome keeps its unique profile. The Demo retains only the two exact `Popen` handles, cleans them in reverse order from one `finally` on pass, failure, cancellation, or startup failure, and never scans or terminates by process name. `final-state.json` records outcome, sanitized failure class, exact fixture identity, per-PID disposition, and explicit `handoff_required` when cleanup cannot be confirmed. Nine deterministic tests cover fresh start, launch arguments, partial-launch cleanup, forced fallback, explicit handoff, failure/cancel, and normal completion. A real run must still verify that no detached application window survives. | Start and end state are both declared. A failed/escaped run closes or clearly hands off its disposable windows, and the next run starts from the same pristine state without touching unrelated user windows. |
| `GDA-HUD-011` | Evidence and promotion | The visual changes have no retained screenshot/video matrix or successful end-to-end run. The latest run is a failure and must not replace prior passing evidence. | Computer Use inspected the synthetic real-Demo-text Decision Card at the current DPI in compact and expanded states: approval/workflow/application/action text and the 2x2 choices were readable; expanded trade-off/safety panes had no bottom clipping; `Esc` removed the window. Those screenshots are session-visible only, not retained repository artifacts. The complete offline gate now passes (`1515 passed, 8 skipped`; Ruff, mypy, docs consistency, and diff checks pass). Presence, multi-DPI, retained screenshots, cleanup-on-real-apps, and a complete Chrome-to-Word run remain. | Each issue has proportional offline tests, a dedicated live smoke where visual behavior matters, one complete retained Chrome-to-Word run, documented DPI/keyboard evidence, full validation gate, and explicit statement that the result remains bounded rather than universal GUI evidence. |
