...
> current-candidate Notepad and fixed Chrome-to-Word integration evidence is
> complete at runtime candidate `d254cd9`; its complete local gate passed and
> publication is the single active action. The run also confirmed that current
> `approved_actions` still prompts for every side effect. `GDA-PRODUCT-016`
> bounded Host-owned risk-tier approval policy is exact next after a clear
> merge; remaining human accessibility/native-control checks follow it, while
...
| `GDA-PRODUCT-015` | Local gate complete; publication active | Current-candidate non-E4 product integration | Runtime commits `6dedc19` and `d254cd9` repaired only observed fixed-workflow composition gaps. One clean Python 3.13 wheel `9589a611...e1423a` passed installed readiness, bounded OpenAI/Notepad Desktop Ask, and fixed OpenAI/Chrome/Word Pre-run Review/Decision Card/save/reopen/receipt/Task Center/cleanup evidence. Complete gate: `2028 passed, 8 skipped`, Ruff, mypy over 138 source files, docs consistency, and diff check. Scope remains exact and [retained](docs/CURRENT_CANDIDATE_PRODUCT_INTEGRATION_EVIDENCE.md); E4, release, broader applications, and old-candidate reuse remain excluded |
| `GDA-PRODUCT-016` | Exact next after PRODUCT-015 merge | Bounded Host-owned risk-tier approval policy | Replace per-side-effect prompting only inside reviewed Host policy: low-risk reversible effects may proceed without a Decision Card, while high-risk, ambiguous, forbidden, or scope-drifted effects still require explicit approval or fail closed. Risk is never model-authored; preserve grounding, budgets, audit, mandatory re-observation, sole Runner/MCP dispatch, and unknown-outcome no-replay |
...
| Offline baseline | Run the current suite; CI publishes the live total. Last full gate: 2026-08-07, all green — `2028 passed, 8 skipped`, Ruff, mypy over 138 source files, docs consistency, and `git diff --check` |
...
| `GDA-PRODUCT-015` | Local gate complete; publication active | Current-candidate non-E4 product integration | Same clean wheel `9589a611...e1423a` passed the bounded Notepad and fixed Chrome-to-disposable-Word product paths, including review/approval, save/reopen, receipt/Task Center, and exact fixture cleanup; two observed composition defects were bounded and regression-tested. The complete `2028 passed, 8 skipped`/Ruff/mypy/docs/diff gate passed. The current every-side-effect approval UX is disclosed, not treated as final risk-tier behavior |
| `GDA-PRODUCT-016` | Exact next after PRODUCT-015 merge | Bounded Host-owned risk-tier approval policy | Let fixed low-risk reversible effects proceed under reviewed Host policy while retaining explicit approval or denial for high-risk/ambiguous effects and every existing authority/recovery invariant |
...
merged through PR #281 as `837a42f`. `GDA-PRODUCT-015` same-wheel
current-candidate non-E4 product integration passed its complete local gate and
is in publication; `GDA-PRODUCT-016` risk-tier approval policy is exact next.
