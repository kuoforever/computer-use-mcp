...
> The user explicitly continued delivery. `GDA-PRODUCT-014` through
> `GDA-PRODUCT-016` are merged and cleaned up; `GDA-PRODUCT-017` automated
> evidence merged through PR #284 as `0f559ec`. Supervised English Narrator
> review repaired one focus-triggered 500-plus-character details dump. The
> cross-surface walkthrough repaired Progress/Presence 200%/400% overflow and
> semantics; ten native cases and the final human visual review passed. One
> corrected fake-only takeover/resume timing run passed released-interval
> input, fresh observation, no post-resume input, and unchanged focus. A
> 30-second notification produced no banner or history while global Windows
> `ToastEnabled=0`; visibility is blocked, not passed. Exact next is intentional
> notification enablement and visibility/retrieval rerun. Other AT/locales,
> physical two-monitor, E4, and release remain separate gates.
...
| `GDA-PRODUCT-017` | Human gates partially complete; notification environment-blocked | Remaining available non-E4 human/native acceptance | PRODUCT-016 merge `7184dee` and PR #284 supplied the complete automatic baseline. Supervised English Narrator review reproduced and repaired a 500-plus-character focus-triggered details dump. A later cross-surface heuristic/cognitive walkthrough accepted the repaired Decision Card, found real Progress/Presence large-text defects, repaired them with a wrapping Document plus real presentation-only Button and measured Presence glyph extents, and the operator accepted the revised 200%/400% presentation. One corrected fake-only human takeover/resume run passed `65.2 ms` request-to-released, released-interval input, `1587.5 ms` resume-to-closed, mandatory fresh observation, no post-resume input, and unchanged focus. A fixed 30-second notification remained unseen and absent from notification-center history while global `ToastEnabled=0`; the human visibility gate is blocked, not passed. [Automatic evidence](docs/PRODUCT017_AUTOMATED_NATIVE_EVIDENCE.md) and [human evidence](docs/PRODUCT017_HUMAN_NATIVE_EVIDENCE.md) retain exact scope and invalid attempts. Other AT/locales remain unclaimed, physical two-monitor is hardware-blocked, and E4/release remain deferred. Exact next: intentionally enable Windows notifications and rerun banner visibility/retrieval |
...
| Offline baseline | Run the current suite; CI publishes the live total. Last full gate: 2026-08-08, all green — `2066 passed, 8 skipped`, Ruff, mypy over 138 source files, docs consistency, and `git diff --check` |
...
| `GDA-PRODUCT-017` | Human gates partially complete; notification environment-blocked | Remaining available non-E4 human/native acceptance | PR #284 merged the complete automatic baseline as `0f559ec`; PR #286 later merged the Narrator/Progress/Presence repairs as `5b2790c`. The bounded English Narrator path, complete one-monitor 200%/400% human visual review, and one fake-only human takeover/resume timing path pass. Notification Shell lifecycle still passes automatically, but a 30-second human attempt produced no banner or history while global `ToastEnabled=0`; visibility is blocked by the current Windows setting. Other AT/locales remain unclaimed, physical two-monitor is hardware-blocked, and E4/release remain deferred. Exact next is intentional notification enablement and human visibility/retrieval rerun |
...
`0f559ec`, and its Narrator/large-text repairs merged through PR #286 as
`5b2790c`; notification visibility remains environment-blocked while the other
named available human gates pass.
