...
> `GDA-PRODUCT-003` is merged through PR #268 as `5eb9182`: one exact-candidate
> Windows/OpenAI/Notepad Desktop Ask result passed after the only observed
> scope-grammar defect was repaired. `GDA-PRODUCT-004` is the single active
> item: ship an installed, model-driven public-browser-to-disposable-Word
> workflow without importing draft PR #231 wholesale. `GDA-CORE-029` closed inside
...
| `GDA-PRODUCT-004` | Active; implementation | Installed `workflow public-web-word` product path with real-model step choice, packaged disposable DOCX template, installed sibling-MCP discovery, durable reopen/render verification, and exact fixture lifecycle; selectively port only required work from draft PR #231 | A clean wheel exposes the command and packaged assets; fresh Chrome observations produce a non-prewritten 2-4 bullet brief, saved/reopened DOCX content and render QA pass, disposable fixtures clean up without manual content correction, and output reports bounded hashes/counts rather than raw source/model text |
| `GDA-PRODUCT-005` | Queued | Exact-candidate integration and release: current-wheel E3/E4, default-on installed action feedback, clean preflight, version/changelog/package metadata, GitHub release wheel and SHA-256 | Same commit and wheel hash pass CI, reviewed provider/desktop gates, the installed product profile defaults `CUMCP_ACTION_FEEDBACK=1` while retaining non-activating/click-through/capture-excluded/content-free behavior, release review, clean install, version check, and rollback/uninstall instructions |
...
| `GDA-PRODUCT-004` | Active; implementation | Model-driven public-browser-to-disposable-Word product workflow | Installed `workflow public-web-word` entry and packaged template, non-prewritten brief from fresh source observations, durable reopen/render verification, exact cleanup, and retained current-main evidence; controlled provider remains test-only |
| `GDA-PRODUCT-005` | Queued | Exact-candidate integration and first GitHub release | Current-wheel E3/E4, default-on installed action feedback under its existing non-activating/click-through/capture-excluded/content-free contract, clean preflight, release review, tagged wheel/digest, clean install and rollback result |
...
| 2026-08-06 | The user kept action feedback opt-in for `GDA-PRODUCT-004` and selected `GDA-PRODUCT-005` as the point where the installed product profile defaults `CUMCP_ACTION_FEEDBACK=1`; the existing non-activating, click-through, capture-excluded, content-free boundary remains fixed. |
