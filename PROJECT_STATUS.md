...
| `GDA-PRODUCT-005` | Queued | Exact-candidate integration and release: current-wheel E3/E4, default-on installed action feedback and progress display, clean preflight, version/changelog/package metadata, GitHub release wheel and SHA-256 | Same commit and wheel hash pass CI, reviewed provider/desktop gates, the installed product profile defaults `CUMCP_ACTION_FEEDBACK=1` and `[operator].progress_enabled=true` while both surfaces retain their existing passive/read-only/no-authority boundaries, release review, clean install, version check, and rollback/uninstall instructions |
...
| `GDA-PRODUCT-005` | Queued | Exact-candidate integration and first GitHub release | Current-wheel E3/E4, default-on installed action feedback and progress display under their existing passive/read-only/no-authority contracts, clean preflight, release review, tagged wheel/digest, clean install and rollback result |
