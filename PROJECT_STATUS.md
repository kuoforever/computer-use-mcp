...
> **Mode: no executable repository item is active.** `GDA-MAINT-010` completed
> through commit `a5c8562`, PR #385, and merge `a7660a0`; both feature-branch
> copies were removed and merge-main CI run `33498294573` passed.
> **Exact next (inactive; separate activation required):** `GDA-MAINT-011` is
...
> activated, resolve only the two independently reproduced typing errors in
> `computer_use_agent.continuation`: the untrusted budget
> `used`-versus-`maximum` comparison and the
> decoded JSON/root payload passed where a mapping is required. Then remove only
...
> expiry/digest/identity validation, persisted bytes, public failures, and
> continuation behavior; add no `Any`, cast, `type: ignore`, or exemption. Prove
...
| --- | --- | --- |
| Core type debt | `GDA-MAINT-010` is merged and closed. The `computer_use_agent.config` whole-module exemption is absent; mypy now has 23 override tables, 23 `ignore_errors = true` entries, and zero target overrides for that module while configured mypy still checks 176 source files. The `computer_use_agent.continuation` exemption remains present; its isolated follow-import diagnostic reports exactly two errors: one `operator` error for untrusted budget `used`-versus-`maximum` values and one `arg-type` error for an untrusted decoded JSON/root payload passed where a mapping is required | Await separate explicit user activation of `GDA-MAINT-011`. If activated, close only those exact two errors, remove only the `computer_use_agent.continuation` override, and synchronize only the derived lock header. Preserve envelope parsing, persisted bytes, public failures, and continuation behavior; add no `Any`/cast/ignore/exemption; prove isolated two to zero and counts `23/23/1` to `22/22/0`; retain configured mypy-176 plus focused/full gates and schema/digests; and stop before `run_lock` or another module |
