...
> **Mode: one executable repository item is active.** `GDA-MAINT-011` was
> activated by the user on 2026-09-01 after the `GDA-MAINT-010` tracker
> closeout merged through PR #386 as `4780a2a` and passed merge-main CI run
> `33500104470`.
> **Exact next:** complete `GDA-MAINT-011` only by resolving the two
> independently reproduced typing errors in `computer_use_agent.continuation`:
> the untrusted budget `used`-versus-`maximum` comparison and the decoded
> JSON/root payload passed where a mapping is required. Preserve the existing
> first-pass budget-field validation order, repeat the comparison reads in
> their existing key order, consume the second-read `_uint` results as typed
> integers, and fail closed if a stateful mapping changes either comparison
> value away from an unsigned integer. This
> bounded TOCTOU hardening intentionally replaces only dynamic second-read
> non-uint outcomes, including bool, negative integer, and any non-integer,
> with `CONTINUATION_INVALID`; stable mappings, dynamic uint comparisons, and
> getter-exception order remain unchanged.
> Explicitly narrow the canonicalized root before envelope construction. Then remove only
> that module's whole-module
> `ignore_errors` override and synchronize only the mechanically derived
> development-lock binding header. Preserve valid envelope parsing,
> expiry/digest/identity validation, persisted bytes, public failures outside
> that explicit TOCTOU transition, and continuation behavior; add no `Any`,
> cast, `type: ignore`, or exemption. Prove
> the isolated diagnostic falls only from two errors to zero, mypy
> override/ignore/target counts change only from `23/23/1` to `22/22/0`,
> configured mypy still checks 176 source files,
...
> `run_lock`, another module, or any Runtime/schema/data-lane/live change.
...
| --- | --- | --- | --- |
| `GDA-MAINT-011` | Active | Resolve only the two continuation typing errors for the untrusted budget `used`-versus-`maximum` comparison and decoded JSON/root payload mapping, harden only the same comparison against stateful-mapping TOCTOU values, characterize parsing and first-failure behavior, remove only that module's whole-module mypy exemption, and synchronize its mechanically derived development-lock binding header | The five-path ceiling is `PROJECT_STATUS.md`, `src/computer_use_agent/continuation.py`, `tests/agent/test_continuation.py`, `pyproject.toml`, and `requirements/dev-py313-windows.lock`. Acceptance requires a HEAD/current behavior differential including custom or exceptional mappings where the public API permits them; retaining the existing budget-field validation order and its `_uint` checks; repeating each used/maximum read in the existing key order before narrowing either value so getter-exception precedence remains exact; retaining stable-mapping bytes and dynamic-uint comparison outcomes while intentionally changing only a second read that changes from a validated uint to bool, a negative integer, or any non-integer into `ContinuationError("CONTINUATION_INVALID")`; explicit fail-closed dict narrowing before envelope construction; exact persisted bytes and all other existing public failures; isolated mypy two errors to zero; override/ignore/target counts `23/23/1` to `22/22/0`; configured mypy over the same 176 source files; lock dependency-body SHA-256 `91125f2184c0e7ce774a0c708c0976cfae6db830bc2590e8ecb7196ba2e480b0`; provider schema SHA-256 `56b2531166b1856481a505f1cf2da5362c3a802c83b49787920abb56ff7e81ee`; core and optional registry digests `3112fbb88ad1398d4dc466cd0b2adff7199ace387d281f9c952ead7b961ed2bb` and `8aae88ff4cb4265ba16f770615ccc2cbd84434e51e03a9569f493a88a443f042`; and focused plus complete repository gates. Stop on any drift outside that explicit TOCTOU transition in validation order, valid parsing, expiry/digest/identity, persisted bytes, public failures, Runtime/schema/registry-digest/data-lane behavior; a file outside the ceiling; a dependency-body edit; `Any`, cast, ignore, or new exemption; `run_lock` or another module; Provider/desktop/application/live expansion; or failing/unresolved PR state |
...
| --- | --- | --- |
| Core type debt | `GDA-MAINT-011` is the only active tranche. At activation, mypy had 23 override tables, 23 `ignore_errors = true` entries, and one target override for `computer_use_agent.continuation`; that module's isolated follow-import diagnostic reported exactly two errors: one `operator` error for untrusted budget `used`-versus-`maximum` values and one `arg-type` error for an untrusted decoded JSON/root payload passed where a mapping is required. HEAD differential also exposed that a stateful budget mapping could change a previously validated uint into bool, a negative integer, or another non-integer before comparison, producing invalid success or raw `TypeError` instead of the fixed continuation failure | Retain the existing budget-field `_uint` validation order; repeat each comparison pair's reads before narrowing either, preserving getter-exception precedence and dynamic uint results while rejecting only dynamic non-uint values with `CONTINUATION_INVALID`; explicitly narrow the canonicalized root mapping; characterize exact stable parsing/first-failure/persisted-byte behavior and the bounded fail-closed transition; remove only the `computer_use_agent.continuation` override; and synchronize only the derived lock header. Prove isolated two to zero, counts `23/23/1` to `22/22/0`, configured mypy-176 plus focused/full gates and fixed lock/schema/digests, and stop before `run_lock` or another module |
...
| --- | --- | --- | --- |
| `GDA-MAINT-011` | Active | Resolve only the two continuation typing errors; preserve valid envelope parsing, stable-mapping first-failure order, persisted bytes, and continuation behavior while hardening only stateful comparison reads that change from validated uints to bool, a negative integer, or any non-integer | Five-path ceiling, the explicit `CONTINUATION_INVALID` TOCTOU transition, and all other acceptance evidence are owned by Current authorization. Stop before `run_lock`, another module, or any Runtime/schema/registry-digest/data-lane/Provider/desktop/application/live expansion |
