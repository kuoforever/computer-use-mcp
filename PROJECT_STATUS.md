...
> `GDA-PRODUCT-016` bounded Host-owned risk-tier approval policy is the single
> active item. It preserves default per-effect approval while the fixed
> public-web-word profile lets only exact Host-validated low-risk steps skip a
> Decision Card; high risk requires exact approval and unknown risk is denied.
> Remaining human accessibility/native-control checks follow it, while
...
| `GDA-PRODUCT-016` | Local and clean-wheel gates complete; publication active | Bounded Host-owned risk-tier approval policy | Commit `ff8f357` preserves default/legacy per-effect approval. The fixed public-web-word v2 profile uses Host-owned risk classification: exact in-scope low-risk reversible effects proceed without a Decision Card; high risk requires exact approval; missing, invalid, ambiguous, forbidden, or scope-drifted classification is denied before dispatch. Risk is never model-authored; grounding, budgets, audit, mandatory re-observation, cooperative control, sole Runner/MCP dispatch, and unknown-outcome no-replay remain mandatory. Complete source gate: `2037 passed, 8 skipped`, Ruff, mypy over 138 source files, docs consistency, and diff check. Clean Python 3.13 wheel `1b1583a5...f9cc7ad` passed normal dependency installation, installed import provenance, public-web-word v2 init/validate/JSON review, exact `7` side-effect / `0` expected high-risk-approval bounds, no acknowledgement authority, and zero output creation without opening provider, MCP, application, or desktop ports |
| `GDA-PRODUCT-017` | Queued after PRODUCT-016 | Remaining available non-E4 human/native acceptance | Run human Narrator and 200%/400% visual review, fixed notification presentation, and native cooperative takeover timing only after functionality is complete. Keep physical two-monitor hardware-blocked and E4/release explicitly separate |
...
| Offline baseline | Run the current suite; CI publishes the live total. Last full gate: 2026-08-07, all green — `2037 passed, 8 skipped`, Ruff, mypy over 138 source files, docs consistency, and `git diff --check` |
...
| `GDA-PRODUCT-016` | Local and clean-wheel gates complete; publication active | Bounded Host-owned risk-tier approval policy | Commit `ff8f357` passed default per-effect approval, fixed low-risk no-prompt authorization, high-risk exact approval, missing/unknown/throwing/invalid classifier denial, the complete `2037 passed, 8 skipped` source gate, and clean wheel `1b1583a5...f9cc7ad` installed init/validate/review v2 smoke while preserving every authority/recovery invariant and opening no external port |
| `GDA-PRODUCT-017` | Queued after PRODUCT-016 | Remaining available non-E4 human/native acceptance | Human assistive-technology/large-text/visual, notification presentation, and native takeover timing evidence; physical two-monitor remains hardware-blocked, E4/release remain deferred |
...
as `3ddc8f8`. `GDA-PRODUCT-016` risk-tier approval policy is active.
