...
| `GDA-HUD-002` | Decision Card layout | The first card is not a compact Codex/Claude-style HUD. The instruction wraps into the timeout, dense technical text is visible immediately, four long buttons stack vertically, the bottom is clipped, and typography/spacing look like a legacy form. | Width/height were reduced and text labels changed, but the underlying layout still exposes the full content pane and switches to one column below 520 client pixels. The live screenshot proves the result is unacceptable. | Default view fits wholly in the work area and shows only lock state, `1/7`, current action, application, countdown, a details affordance, and a 2x2 set of short choices. No overlap, clipping, or scroll is present in compact mode at 100%, 125%, and 150% DPI. |
| `GDA-HUD-003` | Expandable details | “Expand technical details” currently toggles only the evidence pane inside the same crowded layout; it is not a genuine compact/expanded state. | Decision content and evidence remain separate read-only panes, but content is always visible and the window does not intentionally resize between modes. | Compact mode hides both decision trade-offs and digest evidence. Expanding reveals bounded decision details and evidence; collapsing restores the exact compact geometry without losing the pending decision. |
...
| `GDA-HUD-005` | Step semantics | The surfaces mix two different totals: Progress uses the 17-tool-call budget while the card shows seven approval actions. The progress view cannot yet name the exact current Demo item from durable Host state. | Progress text now derives `STEP current/limit` from checkpoint tool-call counts; Demo cards have fixed `n/7` action labels. These are presentation-only partial changes. | The UI clearly labels “workflow step” versus “approval n/7”, names the current fixed action without trusting provider prose, and defines how skipped, failed, verification, and terminal steps affect counts. |
| `GDA-HUD-006` | Progress HUD visual design | The passive progress window has only received a dark fill and accent stripe; its hierarchy, compactness, typography, current-action emphasis, and expand behavior have not been seen or accepted live. | Dark 460x250 rendering and `STEP` text exist in the worktree; only pure rendering tests have passed. | A live passive window remains non-activating and foreground-safe while clearly showing overall progress, current action/phase, application, and expandable sanitized detail. |
