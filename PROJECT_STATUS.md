...
| `GDA-HUD-005` | Step semantics | The surfaces mix different totals: Progress uses the Host tool-call budget while the card shows seven approval actions. The progress view cannot yet name the exact current Demo chapter from durable Host state. | A bounded immutable checklist contract now distinguishes completed, current, waiting-approval, skipped, failed, uncertain, and not-started rows, and rejects contradictory ordering. The Demo has six fixed human-readable Host-owned chapters, separate from seven approvals and the tool-call budget. Native Progress/Decision wiring is not started. | The UI clearly labels “workflow step” versus “approval n/7”, names the current fixed action without trusting provider prose, and defines how skipped, failed, verification, and terminal steps affect counts. |
