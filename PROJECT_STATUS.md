...
> `GDA-PRODUCT-017` is the single active item. Its automatically testable
> native UIA/large-text and notification subset passed; low-risk authorization
> plus cooperative takeover now has an exact composition regression. Human
> Narrator/visual/visible-notification/native-takeover checks still require an
> awake operator and remain `NOT RUN`, while
...
| `GDA-PRODUCT-017` | Automated subset complete; publication active; human gates remain | Remaining available non-E4 human/native acceptance | Ten current native UIA/theme/locale/large-text cases, two Shell notification lifecycles, low-risk Host authorization plus external takeover/fresh-observation composition, and the complete `2038 passed, 8 skipped`/Ruff/mypy/docs/diff gate passed. Human AT/visual/visible-notification/operator-timing remain `NOT RUN`; physical two-monitor remains hardware-blocked, E4/release remain deferred |
...
`GDA-PRODUCT-017` automated non-E4 evidence is active.
