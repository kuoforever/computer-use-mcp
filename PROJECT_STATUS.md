...
> The user explicitly resumed after PR #278. `GDA-PRODUCT-011` localization
> merged through PR #278 as `c58d768`, and both feature branches were removed.
> `GDA-PRODUCT-012` is locally complete and publication is the single active
> action: Host-owned foreground-monitor composition for the three native
> operator surfaces. Personalization, human assistive-technology evidence, E4,
> and release remain separate later gates.
...
| `GDA-PRODUCT-012` | Local gate complete; publication pending | Multi-display composition for native operator surfaces | One immutable Host-selected foreground-monitor bounds/work-area/effective-DPI snapshot now composes Presence, Progress, and Decision Card with negative-coordinate, mixed-DPI, primary-fallback, and failure coverage. Complete local gate: `2005 passed, 8 skipped`, Ruff, mypy over 137 source files, docs consistency, diff check, clean Python 3.13 wheel `3DCA5573...22F4A580`, installed init/validate/doctor/import smoke, and bounded two-locale current-monitor native UIA/no-activation smoke returning `option_deny`. The available machine had one physical monitor, so cross-monitor usability remains unclaimed. Desktop dispatch, coordinate authority, capture scope, and primary-display MVP claims are unchanged |
| `GDA-PRODUCT-013` | Exact next after PRODUCT-012 merge | Bounded operator presentation personalization | Audit the existing operator config, visual tokens, accessibility precedence, and surface layout preferences, then select one smallest strict presentation-only personalization contract before editing. Do not add learned/per-application preferences, model-controlled styling, desktop authority, or a second tracker. Human assistive-technology/E4/release evidence remain later |
...
| Offline baseline | Run the current suite; CI publishes the live total. Last full gate: 2026-08-07, all green — `2005 passed, 8 skipped`, Ruff, mypy over 137 source files, docs consistency, and `git diff --check` |
...
| `GDA-PRODUCT-012` | Local gate complete; publication pending | Multi-display composition | Host-owned foreground-window monitor selection now supplies one validated bounds/work-area/DPI snapshot to native operator placement without changing desktop-action coordinates, capture scope, authority, or replay behavior. The complete offline, clean-wheel, installed, and single-monitor native gates passed; physical cross-monitor evidence remains open |
| `GDA-PRODUCT-013` | Exact next after PRODUCT-012 merge | Bounded operator presentation personalization | Audit existing strict presentation inputs and select one smallest non-authoritative personalization contract before implementation; learned/per-application/model-controlled preferences remain excluded |
...
as `c58d768`. `GDA-PRODUCT-012` multi-display composition is locally complete
and awaiting publication; `GDA-PRODUCT-013` bounded personalization audit is
the exact post-merge next item.
