...
_VERSION_FIELD = "approval_inbox_version"
...
_FIELDS = _RECORD_FIELDS | {_VERSION_FIELD}
_ACTION_LABELS = {
...
def _aware_utc(value: datetime) -> datetime:
    if not isinstance(value, datetime) or value.tzinfo is None or value.utcoffset() is None:
        raise ApprovalInboxError("APPROVAL_INBOX_TIME_INVALID")
...
class ApprovalInboxRecord:
    ...
    def action_label(self) -> str:
        """Derived, never stored, so a record cannot disagree with its action."""

        return _ACTION_LABELS[self.tool_name]
...
class LocalApprovalInbox:
    ...
    def _require_safe_path(self, path: Path) -> None:
        if has_unsafe_ancestor(path, root=self.state_dir):
    ...
    def _decode(payload: object) -> ApprovalInboxRecord:
        if not isinstance(payload, dict) or set(payload) != _FIELDS:
            raise ApprovalInboxError("APPROVAL_INBOX_RECORD_INVALID")
        if payload[_VERSION_FIELD] != APPROVAL_INBOX_VERSION:
            raise ApprovalInboxError("APPROVAL_INBOX_VERSION_UNSUPPORTED")
        # The key set is already exactly _FIELDS, so the remainder is exactly
        # the record's own fields. __post_init__ validates every value.
        fields = {key: payload[key] for key in _RECORD_FIELDS}
        return ApprovalInboxRecord(**fields)  # type: ignore[arg-type]
    ...
    def open(
        ...
        *,
        ...
        expires = _aware_utc(card.expires_at)
        if (
        ...
        # Checked after mkdir so the directory this write lands in is the one
        # that was validated.
        self._require_safe_path(path)
        # Not size-checked: every field is regex- or clock-bounded well under
        # MAX_APPROVAL_INBOX_BYTES, which exists to bound untrusted reads.
        encoded = (
...
class ApprovalAttentionLifecycle:
    """Publish and withdraw supplemental attention for one pending approval.

    Failures propagate. The approval boundary that owns this lane already
    isolates it from authority, so swallowing here as well would only make a
    broken notifier permanently invisible without changing any decision.
    """
    ...
    def close(self, request_id: str) -> None:
        try:
            ...
            # Removing the record is the property that matters: a record left
            # behind would keep presenting a decided request as pending.
...
def build_approval_inbox(
    ...
    *,
    ...
    for path in entries:
        ...
        # Pending first, then soonest expiry. The expiry is already parsed here,
        # so ordering never re-parses a timestamp per comparison.
        rank = (0 if remaining > 0 else 1, expires, record.request_id)
        ordered.append((rank, ApprovalInboxItem(record, status, remaining)))
    ordered.sort(key=lambda entry: entry[0])
    return ApprovalInboxProjection(
        tuple(item for _, item in ordered[:limit]), len(ordered), unavailable
    )
