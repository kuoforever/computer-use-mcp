...
- **Opt-in progress lifecycle.** Ordinary `run`, `resume`, and bounded
  observation-only `plan run` can now drive the passive progress window from
  durable checkpoints on a dedicated Win32 UI thread. The feature defaults
  off, remains read-only, survives human takeover, closes on E-stop/final
  cleanup, and fails silently without affecting the run.
